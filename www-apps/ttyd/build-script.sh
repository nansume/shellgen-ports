...
# Date: 2025-03-01 15:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/www-apps/ttyd/ttyd-1.7.7.ebuild
...
PN=${PN%%_*}
XPN=${XPN:-$PN}
PV="1.7.3"  # BUG: blank screen and it nowork.
PV="1.7.2"
SRC_URI="https://github.com/tsl0922/${PN}/archive/${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
pkginst \
  "dev-db/sqlite3  # for libwebsockets" \
  "dev-libs/json-c" \
  "dev-libs/libuv  # for libwebsockets" \
  "dev-util/cmake" \
  "dev-util/pkgconf" \
  "net-libs/libwebsockets" \
  "net-libs/mbedtls  # for libwebsockets" \
  "sys-devel/binutils" \
  "sys-devel/gcc9" \
  "sys-devel/make" \
  "sys-libs/musl0" \
  "sys-libs/zlib  # for libwebsockets" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  if use !shared && use 'static'; then
    ...
    PKG_CONFIG="${PKG_CONFIG}${PKG_CONFIG:+ }-static --static"
  else
    append-flags -O2
  fi
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -DLWS_MBEDTLS_ENABLED=ON \
    -DLWS_OPENSSL_SUPPORT=OFF \
    -DCMAKE_EXE_LINKER_FLAGS="$(usex static '-static -no-pie -Wl,-s -Wl,-Bsymbolic -Wl,--gc-sections')" \
