...
# Date: 2025-04-05 10:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/guru/dev-db/soci/soci-4.0.3-r3.ebuild
...
XPN=${XPN:-$PN}
PV="4.0.3"
BASE_URI="https://sourceforge.net/projects/soci"
SRC_URI="${BASE_URI}/files/soci/${PN}-${PV}/${PN}-${PV}.tar.gz/download -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
pkginst \
  "dev-db/sqlite3" \
  "dev-libs/cxx-boost  # TODO: bump: to cxx-boost-1.85.0" \
  "dev-util/cmake" \
  "dev-util/pkgconf" \
  "sys-devel/binutils" \
  "sys-devel/gcc9" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  ldd "$(get_libdir)/lib${PN}_core.so" || : die "library deps work... error"
