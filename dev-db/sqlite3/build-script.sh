...
PV_YEAR="2024"
XPV="3460100"
SRC_URI="https://sqlite.org/${PV_YEAR}/${PN}-autoconf-${XPV}.tar.gz -> ${PN}-autoconf-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
IUSE="+static +static-libs +shared (-musl) +fts3 -test +stest +strip"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-autoconf-${XPV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc$(usex static ' -static --static')" CXX="g++$(usex static ' -static --static')"
  ...
  ldd "bin/${PN}3" || { use 'static' && true || die "library deps work... error";}
