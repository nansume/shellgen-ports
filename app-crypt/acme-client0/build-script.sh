...
# Date: 2023-2024, 2024-04-10 19:00 UTC, 2025-06-05 16:00 UTC - last change
...
LICENSE="GPL-2"  # no compat LibreSSL/OpenSSL, however the itself within sources a files to have ISC license.
IFS="$(printf '\n\t')"
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
...
PN=${PN%[0-9]}
PV="1.3.0"
SRC_URI="https://git.sr.ht/~graywolf/${PN}-portable/archive/v${PV}.tar.gz -> ${PN}-fork-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc$(use static && printf ' -static --static')"  # TODO: it remove
  CXX="g++$(use static && printf ' -static --static')"  # TODO: it remove
  ...
  use 'doc' || rm -v -r -- "usr/share/doc/" "usr/"
