...
pkginst \
  ...
  "dev-libs/zrtpcpp-static  # zrtp" \
  "dev-qt/qt5base12-static  # pre: qt5base0" \
  "dev-qt/qt5declarative12-static  # pre: qt5declarative0" \
  "dev-qt/qt5tools12-static  # pre: qt5tools0" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  if use !shared && use 'static'; then
    ...
    append-ldflags -lgpg-error -lgcrypt -lgnutls -lccrtp
    #export LIBS="-lgpg-error -lgcrypt -lgnutls -lccrtp"
  else
    append-flags -O2
  fi
