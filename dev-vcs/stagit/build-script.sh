...
# Date: 2025-05-14 12:00 UTC - last change
...
PV="1.2"
SRC_URI="
  https://codemadness.org/releases/stagit/${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/dev-vcs/${PN}/files/${PN}-1.1-pkg-config.patch
"
USE_BUILD_ROOT="0"
...
pkginst \
  "dev-libs/libgit2" \
  "dev-libs/pcre2  # deps libgit2" \
  "dev-util/pkgconf" \
  "net-libs/llhttp  # deps libgit2" \
  "net-libs/libssh2  # deps libgit2" \
  "net-libs/mbedtls  # deps libgit2" \
  "sys-devel/binutils" \
  "sys-devel/gcc9" \
  "sys-devel/make" \
  "sys-libs/musl" \
  "sys-libs/zlib  # deps libgit2" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  if use !shared && use 'static'; then
    ...
    append-ldflags -lmbedtls -lmbedcrypto -lmbedx509 -lssh2 -lz -lllhttp
    append-ldflags -lpcre2-8 -lpcre2-posix
  else
    append-flags -O2
  fi
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-1.1-pkg-config.patch
  ...
  make \
    DESTDIR="${ED}" \
    PREFIX="${EPREFIX%/}" \
    DOCPREFIX="${EPREFIX%/}"/usr/share/doc/${PN}-${PV} \
