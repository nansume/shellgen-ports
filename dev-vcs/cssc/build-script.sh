...
# Date: 2024-03-16 17:00 UTC - last change
# Build with useflag: -static -man +musl +x32
...
SRC_URI="http://ftp.gnu.org/gnu/cssc/CSSC-${PV}.tar.gz"
LICENSE="GPL-3"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-static -man (+musl) -test +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
PKGNAME=${PN}
ZCOMP="gunzip"
WORKDIR="${PDIR%/}/${SRC_DIR}/CSSC-${PV}"
S="${PDIR%/}/${SRC_DIR}/CSSC-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  use 'man' || rm -r -- "usr/share/"

  use 'static' && LD_LIBRARY_PATH=
  bin/sccs --version || die "binary work... error"

  ldd "bin/sccs" || { use 'static' && true;}
