...
# Date: 2025-04-18 21:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=fluxcomp
# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=flux-git
...
HOMEPAGE="https://github.com/kevleyski/fluxcomp"
...
PN=${PN%[0-9]}
PV="1.4.4.1" # flux r1
PV="1.4.4.2" # fluxcomp r2
#XPV=${PV/-/.}
SRC_URI="https://github.com/deniskropp/flux/archive/master.tar.gz -> ${PN}-${PV}.tar.gz"
SRC_URI="https://github.com/kevleyski/${PN}/archive/master.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  mv -n configure.in -T configure.ac

  test -x "/bin/perl" && { autoreconf -fiv ;}
  ...
  mv -n src/${PROG} -t "${ED}"/bin/ || : die "make install... error"
