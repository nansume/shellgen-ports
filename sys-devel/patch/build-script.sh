...
# Date: 2023-12-29 15:00 UTC - last change
...
SRC_URI="ftp://ftp.gnu.org/gnu/${PN}/${PN}-${PV}.tar.xz"
LICENSE="GPL-3+"
USER=${USER:-root}
USE_BUILD_ROOT='0'
...
IUSE="+static -patch +diet (-musl) -xattr -test +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  if use 'diet'; then
    IUSE="${IUSE} +static"
    pkginst "sys-kernel/linux-headers"
    mkdir -pm 0755 /opt/diet/
    mv -n /usr/include /opt/diet/
