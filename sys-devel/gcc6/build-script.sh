...
PV="6.5.0"
XPV=${PV%.*}
TAG="0fa503e000050fa6c57ab8402f0b6643dfc18cc0"
PATCH_URI1="gitweb.gentoo.org/proj/gcc-patches.git/plain"
PATCH_URI2="git.alpinelinux.org/aports/plain/main/gcc"
SRC_URI="
  http://ftp.gnu.org/gnu/${PN}/${PN}-${PV}/${PN}-${PV}.tar.xz
  https://${PATCH_URI1}/${PV}/gentoo/01_all_default-fortify-source.patch -> ${PN}6-01_default-fortify-source.patch
  ...
  https://gitweb.gentoo.org/proj/gcc-patches.git/plain/${PV}/gentoo/14_all_asan-signal_h.patch
  ...
  https://gitweb.gentoo.org/proj/gcc-patches.git/plain/${PV}/gentoo/23_all_std_pair_ABI.patch
  https://${PATCH_URI2}/020_all_msgfmt-libstdc++-link.patch?id=${TAG}
  https://${PATCH_URI2}/204-linux_libc_has_function.patch?id=${TAG}
  https://${PATCH_URI2}/205-nopie.patch?id=${TAG}
  https://${PATCH_URI2}/320-libffi-gnulinux.patch?id=${TAG}
  https://${PATCH_URI2}/boehm-gc-musl.patch?id=${TAG}
  https://${PATCH_URI2}/fix-gcj-iconv-musl.patch?id=${TAG}
  https://${PATCH_URI2}/fix-gcj-musl.patch?id=${TAG}
  https://${PATCH_URI2}/fix-gcj-stdgnu14-link.patch?id=${TAG}
"
USE_BUILD_ROOT="0"
...
pkginst \
  "dev-libs/gmp" \
  "#dev-libs/isl  # error: 'isl_val_free' was not declared in this scope" \
  "dev-libs/mpc" \
  "dev-libs/mpfr" \
  "sys-apps/file" \
  "sys-devel/binutils" \
...
use 'bootstrap' || {
  if pkg-is "sys-devel/gcc6" > /dev/null; then
    pkginst "sys-devel/gcc6"
  elif pkg-is "sys-devel/gcc7" > /dev/null; then
...
if pkg-is "dev-build/libtool6" > /dev/null; then
  pkginst "dev-build/libtool6"
elif pkg-is "dev-build/libtool7" > /dev/null; then
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  #pkg-is "sys-devel/gcc6" > /dev/null || append-cxxflags -fpermissive
