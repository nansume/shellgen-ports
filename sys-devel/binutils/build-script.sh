...
# Date: 2023-12-15 12:00 UTC - last change
...
SRC_URI="
  http://ftp.gnu.org/gnu/${PN}/${PN}-${PV}.tar.xz
...
IUSE="-bootstrap -rpath -nls +static-libs +shared +static +threads -system-zlib"
IUSE="${IUSE} -default-gold -doc +gold -multitarget -multilib +lto +plugins -test +strip"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # simple test
  LD_LIBRARY_PATH= bin/ld --version
  ldd "bin/ld" || { use 'static' && true;}
