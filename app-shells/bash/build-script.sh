...
export LC_ALL BUILD_USER SRC_DIR
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # bug: <bash: xmalloc: locale.c:84: cannot allocate 39 bytes>
  # musl required: --without-bash-malloc
  . runverb \
  ./configure \
    --prefix=${SPREFIX} \
    --bindir="${SPREFIX%/}/bin" \
    ...
    $(use_enable 'debugger') \
    $(use_enable 'extra' directory-stack) \
    $(use_enable 'extra' restricted) \
    $(use_enable 'extra' select) \
    $(use_enable 'threads') \
    $(use_enable 'rpath') \
    $(use_enable 'nls') \
