...
# Date: 2023-11-21 14:00 UTC - last change
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake \
    ...
    -DCMAKE_INSTALL_BINDIR="${SPREFIX%/}/bin" \
    -DCMAKE_INSTALL_LIBDIR="${SPREFIX%/}/${LIB_DIR}" \
    ...
    -DHAVE_SNAPPY=$(usex 'snappy' ON OFF) \
    -DHAVE_TCMALLOC=$(usex 'tcmalloc' ON OFF) \
    -DLEVELDB_BUILD_TESTS=$(usex 'test' ON OFF) \
    -DCMAKE_SKIP_RPATH=$(usex 'rpath' ON OFF) \
