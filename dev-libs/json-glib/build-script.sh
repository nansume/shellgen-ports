...
# Date: 2024-11-12 08:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/dev-libs/json-glib/json-glib-1.8.0.ebuild
...
XPN=${XPN:-$PN}
PV="1.10.0"
SRC_URI="https://download.gnome.org/sources/json-glib/${PV%.*}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-gtk-doc -introspection -nls -test -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  "app-text/docbook-xml-dtd41" \
  "app-text/docbook-xsl-stylesheets" \
  "dev-build/meson7  # build tool" \
  "#dev-build/muon  # BUG: gnome module implementation is unstable - Build... Failed" \
  "dev-build/samurai  # alternative for ninja" \
  "dev-lang/python38  # deps glib" \
  "dev-libs/expat  # deps glib" \
  "dev-libs/glib74" \
  ...
  "sys-devel/make" \
  "sys-libs/musl" \
  "sys-libs/zlib  # deps glib,libxslt" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Ddocumentation=$(usex 'gtk-doc' enabled disabled) \
