...
# Date: 2024-08-26 11:00 UTC - last change
...
SRC_URI="
  ...
  http://data.gpo.zugaina.org/gentoo/dev-libs/libev/files/libev-4.25-pc.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # bug #411847
  patch -p1 -E < "${FILESDIR}/${PN}-4.25-pc.patch"

  sed -e "/^include_HEADERS/s/ event.h//" -i Makefile.am || die
  ...
  use 'static-libs' || find "$(get_libdir)/" -name '*.la' -type f -delete || die
