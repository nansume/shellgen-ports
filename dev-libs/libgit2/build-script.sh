...
# http://data.gpo.zugaina.org/gentoo/dev-libs/libgit2/libgit2-1.9.0.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-examples -gssapi +ssh -test +threads -trace +static +static-libs -shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/pcre2" \
  "dev-libs/openssl3  # it needed and for <mbedtls>" \
  "dev-util/cmake" \
  "dev-util/pkgconf" \
  "net-libs/llhttp" \
  "net-libs/libssh2" \
  "net-libs/mbedtls  # deps ssh" \
  "sys-devel/binutils" \
  "sys-devel/gcc9" \
  "sys-devel/make" \
  "sys-libs/musl" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's:-Werror::' -i tests/headertest/CMakeLists.txt || die
