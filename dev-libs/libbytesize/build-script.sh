...
# Date: 2024-11-13 18:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# https://data.gpo.zugaina.org/gentoo/dev-libs/libbytesize/libbytesize-2.11.ebuild
...
PV="2.11"
SRC_URI="
  https://github.com/storaged-project/libbytesize/releases/download/${PV}/${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/dev-libs/libbytesize/files/libbytesize-2.4-no_Werror.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-doc -python -test -tools -static -static-libs +shared (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-2.4-no_Werror.patch"
  ...
  find "$(get_libdir)/" -name "*.la" -type f -delete || die
