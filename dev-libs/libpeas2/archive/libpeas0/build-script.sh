...
# http://data.gpo.zugaina.org/gentoo/dev-libs/libpeas/libpeas-1.36.0.ebuild
...
PV="1.36.0"
SLOT="0"
SRC_URI="https://download.gnome.org/sources/libpeas/${PV%.*}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-glade -gtk -gtk-doc -lua -python -vala -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/expat  # deps meson,glib" \
  "dev-libs/glib74" \
  "#dev-libs/gobject-introspection  # BUG: not found" \
