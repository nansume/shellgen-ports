...
# Date: 2025-04-22 17:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=directfb2-lite-git

export XPN PF PV WORKDIR BUILD_DIR PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export CC CXX PKG_CONFIG PKG_CONFIG_LIBDIR PKG_CONFIG_PATH

DESCRIPTION="Lightweight Toolkit Enabler is a simple user interface library on top of DirectFB"
...
PN="LiTE"
PV="0.9.1p20250422"
SRC_URI="https://github.com/directfb2/LiTE/archive/master.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/lite.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/lite.pc || die

  ldd "$(get_libdir)"/liblite.so || die "library deps work... error"
