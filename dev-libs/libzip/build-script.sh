...
# Date: 2024-09-06 09:00 UTC - last change
...
XPN=${XPN:-$PN}
PV="1.10.1"
SRC_URI="https://libzip.org/download/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-bzip2 -gnutls -lzma -mbedtls -ssl -static-libs -test -tools -zstd +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed \
    -e "3s|^bindir=.*|bindir=/bin|;t" \
    -e "4s|^libdir=.*|libdir=/$(get_libdir)|;t" \
