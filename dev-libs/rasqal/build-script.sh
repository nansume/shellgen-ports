...
# https://data.gpo.zugaina.org/gentoo/dev-libs/rasqal/rasqal-0.9.33-r2.ebuild
...
SRC_URI="
  ...
  http://data.gpo.zugaina.org/gentoo/dev-libs/rasqal/files/rasqal-0.9.33-configure-clang16.patch
  http://data.gpo.zugaina.org/gentoo/dev-libs/rasqal/files/rasqal-0.9.33-configure-c99.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+crypt -gmp +mhash -pcre -static-libs -test -xml +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-0.9.33-configure-clang16.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-0.9.33-configure-c99.patch
