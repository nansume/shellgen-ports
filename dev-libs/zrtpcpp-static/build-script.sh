...
# Date: 2024-09-13 12:00 UTC, 2025-05-21 19:00 UTC - last change
...
USE="${USE} +static-libs -shared"  # force set build-flags
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/ucommon  # deps ccrtp (optional)" \
  "dev-libs/ucommon-static  # deps ccrtp" \
  "dev-util/cmake" \
  "dev-util/pkgconf" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake \
    ...
    -D AXO=$(usex 'axo' ON OFF) \
    -D CCRTP=$(usex 'ccrtp' ON OFF) \
    -D CORE_LIB=$(usex 'core' ON OFF) \
    -D NO_CACHE=$(usex 'empty' ON OFF) \
    -D SDES=$(usex 'sdes' ON OFF) \
    -D SQLCIPHER=$(usex 'sqlcipher' ON OFF) \
    -D SQLITE=$(usex 'sqlite' ON OFF) \
    -D TIVI=$(usex 'tivi' ON OFF) \
    -D BUILD_STATIC=$(usex 'static-libs' ON OFF) \
