...
# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=directfb2-lirc-git

export XPN PF PV WORKDIR BUILD_DIR PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export CC CXX PKG_CONFIG PKG_CONFIG_LIBDIR PKG_CONFIG_PATH

DESCRIPTION="LIRC input driver for DirectFB2"
...
pkginst \
  ...
  "sys-libs/musl0" \
  "sys-libs/zlib  # deps meson,libpng" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/directfb-inputdriver-lirc.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/directfb-inputdriver-lirc.pc || die

  ldd "$(get_libdir)"/directfb-2.0-0/inputdrivers/libdirectfb_lirc.so || die "library deps work... error"
