...
# Date: 2025-05-03 12:00 UTC - last change
# Build with useflag: -static -static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +noarch

# http://data.gpo.zugaina.org/gentoo/dev-cpp/msgpack-cxx/msgpack-cxx-6.1.1.ebuild
...
PN=${PN%[0-9]}
PV="6.1.1"
SRC_URI="https://github.com/msgpack/msgpack-c/releases/download/cpp-${PV}/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+boost -doc -examples -test (+musl) -stest -strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/cxx-boost  # cxx-boost[context]" \
  "#dev-libs/msgpack2" \
  "dev-util/cmake" \
  "dev-util/pkgconf" \
  "sys-devel/binutils" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake -B build -G Ninja \
    ...
    -DMSGPACK_USE_BOOST=$(usex 'boost' ON OFF) \
    -DMSGPACK_USE_X3_PARSE=$(usex 'boost' ON OFF) \
