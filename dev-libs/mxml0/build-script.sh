...
# Date: 2024-07-13 21:00 UTC - last change
...
#PN="<pkgname> ${PN%-*}"
PV="3.3.1"
SRC_URI="https://github.com/michaelrsweet/${PN}/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+static-libs +threads +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc$(usex static ' --static')" _AR="ar"
  ...
  ./configure \
    ...
    --with-docdir="${EPREFIX%/}"/usr/share/doc/${PN}-${PV} \
    --host=$(tc-chost) \
    --build=$(tc-chost) \
    $(use_enable 'threads') \
    $(use_enable 'shared') \
  ...
  make DESTDIR="${ED}" DSTROOT="${ED}" install || die "make install... error"

  cd "${ED}/" || die "install dir: not found... error"

  strip --verbose --strip-all "$(get_libdir)/"lib${PN}.so.?.*
  if use 'static-libs'; then
