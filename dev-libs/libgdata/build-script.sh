...
# http://data.gpo.zugaina.org/gentoo/dev-libs/libgdata/libgdata-0.18.1-r1.ebuild
...
PV="0.17.9"
PV="0.18.1"
SRC_URI="https://download.gnome.org/sources/libgdata/${PV%.*}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e '/subdir.*demos/d' -i ${BUILD_DIR}/meson.build || die
  ...
  meson setup \
    ...
    -Doauth1="disabled" \
    -Dalways_build_tests=$(usex 'test' true false) \
