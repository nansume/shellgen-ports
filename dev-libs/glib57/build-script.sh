#!/bin/sh
# Copyright (C) 2023-2024 Artjom Slepnjov, Shellgen
# License GPLv3: GNU GPL version 3 only
# http://www.gnu.org/licenses/gpl-3.0.html
# Date: 2024-04-07 20:00 UTC - last change
# Build with useflag: -static-libs +shared -doc +xstub -diet +musl +stest +strip +x32

export USER XPN PF PV WORKDIR S PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export PKG_CONFIG_LIBDIR PKG_CONFIG_PATH PYTHON

NL="$(printf '\n\t')"; NL=${NL%?}
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
LC_ALL="C"
CATEGORY="${CATEGORY:-${11:?required <CATEGORY>}}"
PN="${PN:-${12:?required <PN>}}"
XPN=${PN}
XPN="${6:-${XPN:?}}"
PN=${PN%-*}
PV="2.56.4"
PV="2.57.1"
XPV="${PV%.*}"
DESCRIPTION="The GLib library of C routines"
HOMEPAGE="https://www.gtk.org/"
SRC_URI="
  ftp://ftp.acc.umu.se/pub/GNOME/sources/${PN}/${XPV}/${PN}-${PV}.tar.xz
  http://data.gpo.zugaina.org/didos/dev-libs/glib/files/2.56.2-quark_init_on_demand.patch
  http://data.gpo.zugaina.org/didos/dev-libs/glib/files/2.56.2-gobject_init_on_demand.patch
"
LICENSE="LGPL-2.1+"
USER=${USER:-root}
USE_BUILD_ROOT="0"
BUILD_CHROOT=${BUILD_CHROOT:-0}
PDIR=$(pkg-rootdir)
DPREFIX="/usr"
INCDIR="${DPREFIX}/include"
INSTALL_OPTS="install"
HOSTNAME="linux"
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-static-libs +shared +musl -debug +stest -test +strip"
IUSE="${IUSE} -dbus -elf -fam -doc -gtk-doc -man -mime -mount -selinux +xstub"
IUSE="${IUSE} -sysprof -systemtap -utils -xattr"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
EPREFIX=${SPREFIX}
P="${P:-${XPWD##*/}}"
SN=${P}
CATEGORY=${11:-$CATEGORY}
PN="${PN:-${P%%_*}}"
PN=${12:-$PN}
PORTS_DIR=${PWD%/$P}
DISTDIR="/usr/distfiles"
DISTSOURCE="${PDIR%/}/sources"
FILESDIR=${DISTSOURCE}
INSTALL_DIR="${PDIR%/}/install"
ED=${INSTALL_DIR}
SDIR="${PDIR%/}/${SRC_DIR}"
PF=$(pfname 'src_uri.lst' "${SRC_URI}")
PKGNAME=${PN}
ZCOMP="unxz"
WORKDIR="${PDIR%/}/${SRC_DIR}/${PN}-${PV}"
S="${PDIR%/}/${SRC_DIR}/${PN}-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG_LIBDIR="/${LIB_DIR}/pkgconfig"
PKG_CONFIG_PATH="${PKG_CONFIG_LIBDIR}:/lib/pkgconfig:/usr/share/pkgconfig"
ABI_BUILD="${ABI_BUILD:-${1:?}}"
BUILD_CHROOT="${7:-${BUILD_CHROOT:?}}"
USE_BUILD_ROOT=${9:-$USE_BUILD_ROOT}
PYTHON="true"

if test "X${USER}" != 'Xroot'; then
  mksrc-prepare
elif test "${BUILD_CHROOT:=0}" -eq '0'; then
  PATH="${PATH:+${PATH}:}${PDIR}/misc.d:${PDIR}/etools.d"
elif test "${BUILD_CHROOT:=0}" -ne '0'; then
  PATH="$(xpath):${PDIR%/}/misc.d:${PDIR%/}/etools.d"
  printf %s\\n "PATH='${PATH}'" "PDIR='${PDIR}'"
fi

. "${PDIR%/}/etools.d/"build-functions

chroot-build || die "Failed chroot... error"

pkginst \
  "dev-libs/libffi  # required" \
  "#dev-libs/pcre  # optional" \
  "dev-util/pkgconf" \
  "sys-apps/file" \
  "sys-devel/binutils" \
  "sys-devel/gcc" \
  "#sys-devel/gettext  # or xstub fix" \
  "sys-devel/make" \
  "sys-kernel/linux-headers  # optional (too recomended)" \
  "sys-libs/musl" \
  "sys-libs/zlib  # required" \
  || die "Failed install build pkg depend... error"

build-deps-fixfind

. "${PDIR%/}/etools.d/"ldpath-apply
. "${PDIR%/}/etools.d/"path-tools-apply

if { use 'xstub' && test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  printf '#!/bin/sh' > /bin/xutils-stub
  chmod +X /bin/xutils-stub
  ln -sf xutils-stub /bin/msgfmt
fi

netuser-fetch "${SRC_URI}" || die "Failed fetch sources... error"
sw-user || die "Failed package build from user... error"

if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  exit
elif test "X${USER}" != 'Xroot'; then
  renice -n '19' -u ${USER}

  cd "${FILESDIR}/" || die "distsource dir: not found... error"

  ${ZCOMP} -dc "${PF}" | tar -C "${PDIR%/}/${SRC_DIR}/" -xkf - || exit &&
  printf %s\\n "${ZCOMP} -dc ${PF} | tar -C ${PDIR%/}/${SRC_DIR}/ -xkf -"

  cd "${WORKDIR}/" || die "workdir: not found... error"

  printf %s\\n "Configure directory: PWD='${PWD}'... ok"

  case $(tc-chost) in
    *'-linux-musl'*)
      # Musl fix
      patch -p1 -E < "${FILESDIR}/2.56.2-quark_init_on_demand.patch"
      patch -p1 -E < "${FILESDIR}/2.56.2-gobject_init_on_demand.patch"
    ;;
  esac

  case $(tc-abi-build) in
    'x32')   append-flags -mx32 -msse2 ;;
    'x86')   append-flags -m32         ;;
    'amd64') append-flags -m64 -msse2  ;;
  esac
  if use 'static-libs'; then
    append-flags -Os
    append-ldflags -Wl,--gc-sections
    append-cflags -ffunction-sections -fdata-sections
  else
    append-flags -O2
  fi
  append-flags -fno-stack-protector -no-pie -g0 -march=$(arch | sed 's/_/-/')

  use 'strip' && INSTALL_OPTS="install-strip"

  IFS=${NL}

  #--enable-debug=minimum \

  . runverb \
  ./configure \
    CC="gcc" \
    CXX="g++" \
    --prefix="${EPREFIX%/}" \
    --bindir="${EPREFIX%/}/bin" \
    --libdir="${EPREFIX%/}/$(get_libdir)" \
    --includedir="${INCDIR}" \
    --datarootdir="${DPREFIX}/share" \
    --host=$(tc-chost) \
    --build=$(tc-chost) \
    $(usex 'debug' --enable-debug=yes) \
    $(use_enable 'doc' gtk-doc-html) \
    $(use_enable 'fam') \
    $(use_enable 'doc' gtk-doc) \
    $(use_enable 'mount' libmount) \
    $(use_enable 'man') \
    $(use_enable 'xattr') \
    --with-threads=posix \
    --disable-libelf \
    --disable-compile-warnings \
    --with-pcre="internal" \
    $(use_enable 'shared') \
    $(use_enable 'static-libs' static) \
    CFLAGS="${CFLAGS}" \
    $(test -n "${LDFLAGS}" && printf "LDFLAGS=${LDFLAGS}") \
    || die "configure... error"

  make -j "$(nproc)" || die "Failed make build"

  . runverb \
  make DESTDIR="${ED}" ${INSTALL_OPTS} || die "make install... error"

  cd "${ED}/" || die "install dir: not found... error"

  # Do not install charset.alias even if generated, leave it to libiconv
  rm -- "$(get_libdir)/charset.alias"

  # Don't install gdb python macros, bug 291328
  rm -r -- "usr/share/gdb/" "usr/share/glib-2.0/gdb/"

  # Completely useless with or without USE static-libs, people need to use pkg-config
  find "$(get_libdir)" -name '*.la' -delete || die

  rm -r -- "usr/share/bash-completion/" "$(get_libdir)/locale/"

  exit 0
fi

cd "${ED}/" || die "install dir: not found... error"

pkg-perm

INST_ABI="$(tc-abi-build)" PN=${XPN} pkg-create-cgz
