...
# Date: 2025-04-25 10:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# TODO: add build static-libs

# http://data.gpo.zugaina.org/gentoo/dev-libs/libinput/libinput-1.28.1.ebuild
...
XPN=${XPN:-$PN}
PV="1.28.1"
HASH="4f7b4ef0e4eb5d569df36be387579858eba349bb"  # for name builddir
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-doc -input_devices_wacom -test -static-libs +shared +nopie (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-${PV}-${HASH}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e "s@, '-Werror'@@" -i meson.build || die  #744250
