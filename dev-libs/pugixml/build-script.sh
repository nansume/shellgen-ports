...
# Date: 2025-06-13 15:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/dev-libs/pugixml/pugixml-1.15.ebuild
...
XPN=${XPN:-$PN}
PV="1.15"
SRC_URI="https://github.com/zeux/${PN}/archive/refs/tags/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -D PUGIXML_BUILD_TESTS=$(usex 'test' ON OFF) \
