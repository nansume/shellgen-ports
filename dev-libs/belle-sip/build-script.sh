...
# Date: 2025-04-04 19:00 UTC - last change
# Build with useflag: static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/nest/dev-libs/belle-sip/belle-sip-5.3.86.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-test -tunnel -zeroconf -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  grep " -L/usr/.*" < $(get_libdir)/pkgconfig/${PN}.pc
  sed \
    -e "s| -L/usr/.* | -L/$(get_libdir) |" \
