...
# Date: 2024-04-17 21:00 UTC - last change
# Build with useflag: -static-libs +shared +ipv6 -doc -xstub +musl +stest +strip +x32
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-static-libs +shared (+musl) +ipv6 -doc -xstub -debug +stest (-test) +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
PKGNAME=${PN}
ZCOMP="gunzip"
WORKDIR="${PDIR%/}/${SRC_DIR}/${PN}-${PV}/${PN}"
S="${PDIR%/}/${SRC_DIR}/${PN}-${PV}/${PN}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  case $(tc-chost) in
    *-"musl"*)
      patch -p1 -E < "${FILESDIR}/nspr-4.21-ipv6-musl-support.patch"
      patch -p1 -E < "${FILESDIR}/nspr-4.35-bgo-905998-lfs64-musl.patch"
    ;;
  esac
  ...
  use 'static-libs' || rm -- $(get_libdir)/*.a || die "failed to remove static libraries."
