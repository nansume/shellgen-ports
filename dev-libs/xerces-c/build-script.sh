...
# Date: 2025-04-04 22:00 UTC - last change
...
PV="3.3.0"
SRC_URI="
  mirror://apache/xerces/c/3/sources/${PN}-${PV}.tar.xz
  http://data.gpo.zugaina.org/gentoo/dev-libs/${PN}/files/${PN}-3.2.4-strict-aliasing.patch
  http://data.gpo.zugaina.org/gentoo/dev-libs/xerces-c/files/${PN}-3.2.5-cxx17.patch
  http://data.gpo.zugaina.org/gentoo/dev-libs/xerces-c/files/50xerces-c
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+cpu_flags_x86_sse2 -curl -doc -examples +iconv -icu -static-libs"
IUSE="${IUSE} -test +threads +sse2 +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-3.2.4-strict-aliasing.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-3.2.5-cxx17.patch  # bug 931105
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -DCMAKE_INSTALL_DOCDIR="${EPREFIX%/}/usr/share/doc/${PN}-${PV}" \
  ...
  use 'stest' && { bin/${PROG} -? || : die "binary work... error";}
