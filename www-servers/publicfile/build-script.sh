...
DESCRIPTION="publicfile supplies files to the public through HTTP and FTP"
HOMEPAGE="http://cr.yp.to/publicfile/install.html"
...
PV="0.52"
SRC_URI="
  http://cr.yp.to/publicfile/${PN}-${PV}.tar.gz
  http://mdcc.cx/pub/publicfile/${PN}_${PV}-11.debian.tar.xz
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  use 'diet' && cp -nl usr/include/sys/*.h -t opt/diet/include/sys/
  ...
  for F in "${WORKDIR}/debian/patches/"*".patch" "${PDIR%/}/patches/"*".diff"; do
    case ${F##*/} in 'errno.patch') continue;; esac
    test -f "${F}" && gpatch -p1 -E < "${F}"
  done

  test -e 'conf-cc' && printf %s\\n "${CC} ${CFLAGS}" > conf-cc
  test -e 'conf-ld' && printf %s\\n "${CC} ${LDFLAGS}" > conf-ld
  test -e 'conf-home' && printf %s\\n "/bin" > conf-home

  . runverb \
  make -j "$(nproc)" || die "Failed make build"

  mkdir -m 0755 -- "${ED}/bin/"
  mv -n ${PROG} ftpd utime -t "${ED}/bin/"
  printf %s\\n "Install: ${PN}... ok"
