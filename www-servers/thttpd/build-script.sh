...
XPN=${XPN:-$PN}
PV="2.29"
SRC_URI="http://www.acme.com/software/thttpd/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc$(usex static ' -static --static') ${CFLAGS}"
  ...
  WEBDIR="/var/www/http"
  CGIBINDIR="/var/www/http/cgi-bin"

  mkdir -pm 0755 -- "${ED}"/sbin/ "${ED}"${CGIBINDIR}
  mv -n ${PN} -t "${ED}"/sbin/
  mv -n extras/htpasswd extras/makeweb extras/syslogtocern -t "${ED}"/sbin/
  mv -n cgi-src/phf cgi-src/redirect cgi-src/ssi -t "${ED}"${CGIBINDIR}
  printf %s\\n "Install: ${PN} sbin/"

  cd "${ED}/" || die "install dir: not found... error"

  mv -n sbin/htpasswd sbin/${PN}-htpasswd
  mv -n sbin/makeweb sbin/${PN}-makeweb
