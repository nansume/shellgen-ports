...
# Date: 2023-12-30 22:00 UTC - last change
...
SRC_URI="
  http://acme.com/software/${XPN}/${XPN}_14Aug2014.tar.gz
  http://deb.debian.org/debian/pool/main/m/${PN}/${PN}_${PV}-2.1.debian.tar.xz
"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  #  printf %s\\n "file: ${F}"
  #  case ${F} in portability|features|*/discard_env|*/sd_daemon|*/libtirpc_|*'/#') continue;; esac
  #  case ${F} in */makefile) ;; *) continue;; esac
  #  patch -p1 -E < "${F}"
  #done
  for F in "${PDIR%/}/patches/"*".diff"; do
    test -f "${F}" && patch -p1 -E < "${F}"
  done
  #patch -p1 -E < "${PDIR%/}/patches/${XPN}-noservid.diff"
  ...
  mkdir -pm 0755 "${ED}/sbin/" "${ED}/usr/share/man/man8/"
  ...
  use 'strip' && strip --verbose --strip-all "sbin/${XPN}"
