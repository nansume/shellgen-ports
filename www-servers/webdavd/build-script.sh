...
# Date: 2025-06-23 01:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=webdav-daemon
...
DESCRIPTION="webdavd is a WebDAV server"
...
SPN="WebDAV-Daemon"
PV="1.2p20231014"
SRC_URI="https://github.com/couling/WebDAV-Daemon/archive/master.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${SPN}-master"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e '/^#include <stdarg.h>$/a #include <time.h>' -i shared.h
  sed -e '/^#include <errno.h>$/a #include <stdlib.h>' -i configuration.c

  make -j "$(nproc)" CC="${CC}" || die "Failed make build"

  mkdir -pm 0755 -- "${ED}/bin/"
  mv -n build/rap build/${PN} -t "${ED}"/bin/ || die "Install... error"
  printf %s\\n "Install: ${PN}... ok"
