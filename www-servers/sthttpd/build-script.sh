...
XPN=${XPN:-$PN}
PV="2.27.1"
SRC_URI="https://git.alpinelinux.org/aports/plain/testing/${PN}"
SRC_URI="
  https://github.com/blueness/sthttpd/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  ${SRC_URI}/${PN}-webgroup.patch -> ${PN}-${PV}-webgroup.patch
  ${SRC_URI}/${PN}-makeweb-notsuid.patch -> ${PN}-${PV}-makeweb-notsuid.patch
  ${SRC_URI}/libhttpd-c.patch -> ${PN}-${PV}-libhttpd-c.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-libhttpd-c.patch

  test -x "/bin/perl" && sh autogen.sh
