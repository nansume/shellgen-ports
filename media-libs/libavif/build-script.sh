...
# http://data.gpo.zugaina.org/gentoo/media-libs/libavif/libavif-1.2.0.ebuild
...
XPN=${XPN:-$PN}
PV="1.2.0"
SRC_URI="https://github.com/AOMediaCodec/libavif/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -D AVIF_CODEC_LIBGAV1=OFF \
    -D AVIF_CODEC_AOM=$(usex 'aom' ON OFF) \
    -D AVIF_CODEC_DAV1D=$(usex 'dav1d' ON OFF) \
    -D AVIF_ZLIBPNG=ON \
    -D AVIF_JPEG=ON \
    -D AVIF_BUILD_GDK_PIXBUF=$(usex 'gdk-pixbuf' ON OFF) \
    -D AVIF_ENABLE_WERROR=OFF \
    -D AVIF_CODEC_RAV1E=$(usex 'rav1e' ON OFF) \
    -D AVIF_CODEC_SVT=$(usex 'svt-av1' ON OFF) \
    -D AVIF_LIBYUV=$(usex 'libyuv' ON OFF) \
    -D AVIF_BUILD_EXAMPLES=$(usex 'examples' ON OFF) \
    -D AVIF_BUILD_APPS=$(usex 'extras' ON OFF) \
    -D AVIF_BUILD_TESTS=$(usex 'test' ON OFF) \
    -D AVIF_GTEST=$(usex 'extras' $(usex 'test' SYSTEM OFF) OFF) \
  ...
  sed -e 's|${prefix}||;s|${exec_prefix}||' -i $(get_libdir)/pkgconfig/${PN}.pc
