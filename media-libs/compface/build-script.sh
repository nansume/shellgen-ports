...
# Date: 2024-07-09 21:00 UTC - last change
...
PV="1.5.2"
SRC_URI="
  http://ftp.xemacs.org/pub/xemacs/aux/${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/media-libs/compface/files/compface-gentoo.patch
  http://data.gpo.zugaina.org/gentoo/media-libs/compface/files/compface-modern-c-porting.patch
"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/compface-gentoo.patch"
  patch -p1 -E < "${FILESDIR}/compface-modern-c-porting.patch"
  ...
  mv -n "${BUILD_DIR}/"xbm2xface.pl "${ED}"/bin/xbm2xface
  chmod +x bin/xbm2xface

  use 'static' && LD_LIBRARY_PATH=
  use 'stest' && { bin/${PN} -v || true;}
