...
# Date: 2025-04-13 17:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/libva/libva-2.22.0-r1.ebuild
...
PV="2.22.0"
SLOT="2"
SRC_URI="https://github.com/intel/libva/archive/${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-glx -wayland +X +shared +nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dbuildtype="release" \
    -Ddriverdir="$(get_libdir)/va/drivers" \
    -Ddisable_drm=false \
    -Dwith_x11=$(usex 'X' yes no) \
    -Dwith_glx=$(usex 'X' $(usex 'glx' yes no) no) \
    -Dwith_wayland=no \
    -Denable_docs=false \
