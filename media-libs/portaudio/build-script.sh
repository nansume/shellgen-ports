...
# Date: 2024-11-18 16:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl -stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/portaudio/portaudio-19.07.00-r2.ebuild
...
DATE="20210406"
SRC_URI="
  http://files.portaudio.com/archives/pa_stable_v${PV}_${DATE}.tgz
  https://dev.gentoo.org/~sam/distfiles/${CATEGORY}/${PN}/${PN}-${XPV}-audacity.patch.bz2
  http://data.gpo.zugaina.org/gentoo/media-libs/${PN}/files/${PN}-19.06.00-AR.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  PF="${FILESDIR}/${PN}-${XPV}-audacity.patch.bz2"
  # Obtained from Fedora this time, previous one was from Debian
  bunzip2 -dc "${PF}" | patch -p1 -E
  # bug #720966, trigger reconf
  patch -p1 -E < "${FILESDIR}"/${PN}-19.06.00-AR.patch
  ...
  use 'strip' &&
  strip --verbose --strip-all "$(get_libdir)"/lib${PN}.so.${SYMVER} "$(get_libdir)"/lib${PN}cpp.so.0.0.12
