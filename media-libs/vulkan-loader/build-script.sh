...
# Date: 2025-04-26 08:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/vulkan-loader/vulkan-loader-1.4.309.0.ebuild
...
XPN=${XPN:-$PN}
PV="1.4.309.0"
SRC_URI="https://github.com/KhronosGroup/Vulkan-Loader/archive/vulkan-sdk-${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-layers -wayland +X -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/Vulkan-Loader-vulkan-sdk-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
pkginst \
  ...
  "x11-libs/libxext" \
  "x11-libs/libxrandr" \
  "x11-libs/libxrender  # for libxrandr" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's|${prefix}||;s|${exec_prefix}||' -i $(get_libdir)/pkgconfig/${PN%-*}.pc || die
