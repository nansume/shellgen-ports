...
# Date: 2024-09-10 17:00 UTC - last change
...
XPN=${XPN:-$PN}
PV="0.16.3"
SRC_URI="https://codeberg.org/tenacityteam/libid3tag/archive/${PV}.tar.gz -> ${PN}-${PV}.codeberg.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed \
    -e "2s|^includedir=.*|includedir=/usr/include|;t" \
    -e "3s|^libdir=.*|libdir=/$(get_libdir)|;t" \
    -i ${ED}/$(get_libdir)/pkgconfig/*.pc || die
