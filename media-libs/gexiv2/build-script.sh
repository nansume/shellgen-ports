...
# Date: 2025-04-09 17:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs -nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/gexiv2/gexiv2-0.14.3.ebuild
...
XPN=${XPN:-$PN}
PV="0.14.3"
SRC_URI="https://download.gnome.org/sources/gexiv2/0.14/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-gtk-doc +introspection -python -test -vala -tools +static-libs +shared -nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "#dev-lang/vala" \
  "dev-libs/expat  # deps exiv2,meson" \
  "dev-libs/glib74" \
  "dev-libs/gobject-introspection" \
  "dev-libs/libffi  # for glib,meson" \
  "#dev-libs/libxml2-1  # for vala" \
  "#dev-libs/libxslt  # for vala" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ln -s "/$(get_libdir)" "/usr/$(get_libdir)"  # FIX: /usr/libx32/zlib.so - not found
