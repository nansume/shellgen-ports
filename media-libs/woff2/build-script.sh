...
# Date: 2024-10-28 21:00 UTC - last change
...
PV="1.0.2"
SRC_URI="
  https://github.com/google/${PN}/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/media-libs/woff2/files/woff2-1.0.2-aliasing.patch
  http://data.gpo.zugaina.org/gentoo/media-libs/woff2/files/woff2-1.0.2-gcc15.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-aliasing.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-1.0.2-gcc15.patch
