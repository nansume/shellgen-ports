...
# Date: 2025-05-28 11:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/libv4l/libv4l-1.28.1.ebuild
...
SPN="v4l-utils"
PV="1.30.1"
SRC_URI="https://linuxtv.org/downloads/v4l-utils/${SPN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
pkginst \
  ...
  "sys-libs/argp-standalone  # !glibc" \
  "sys-libs/musl" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  use 'qt6' || sed -e "/^dep_qt6/s/'qt6/&DiSaBlEd/" -i meson.build || die
  ...
  meson setup \
    ...
    -D mandir="${EPREFIX%/}/usr/share/man" \
    -D systemdsystemunitdir="/lib/systemd/unit.d" \
    -D udevdir="lib/udev" \
    -D wrap_mode="nodownload" \
    -D buildtype="release" \
    -D bpf=$(usex 'bpf' enabled disabled) \
    -D gconv=disabled \
    -D jpeg=$(usex 'jpeg' enabled disabled) \
    -D libdvbv5=$(usex 'dvb' enabled disabled) \
    -D v4l2-tracer=$(usex 'tracer' enabled disabled) \
    -D v4l-utils=$(usex 'utils' true false) \
    -D doxygen-doc=$(usex 'doc' enabled disabled) \
    -D doxygen-html=$(usex 'doc' true false) \
    -D doxygen-man=$(usex 'doc' true false) \
    -D qv4l2=disabled \
    -D qvidcap=disabled \
    -D b_pie="false" \
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/${PN}2.pc
  ...
  #ldd "$(get_libdir)"/libv4l2.so || : die "library deps work... error"
