...
# Date: 2025-05-30 12:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/rnnoise/rnnoise-0.4.1_p20210122-r1.ebuild
...
SRC_URI="https://gitlab.xiph.org/xiph/rnnoise/-/archive/${COMMIT}/rnnoise-${COMMIT}.tar.bz2 -> ${PN}-${PV}.tar.bz2"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-doc -examples +static-libs +shared (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-${COMMIT}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  rm -- "${ED}/usr/share/doc/${PN}-${PV}/COPYING" || die
  use 'doc' || rm -r -- "${ED}"/usr/share/doc/ "${ED}"/usr/share/
  find "$(get_libdir)/" -name '*.la' -delete || die
  use 'examples' && dobin examples/.libs/rnnoise_demo

  ldd "$(get_libdir)"/lib${PN}.so.${SYMVER} || die "library deps work... error"
