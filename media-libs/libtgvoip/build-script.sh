...
# Date: 2025-05-29 16:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/libtgvoip/libtgvoip-2.4.4_p20240706.ebuild
...
BASE_URI="data.gpo.zugaina.org/gentoo/media-libs/libtgvoip"
SRC_URI="
  https://github.com/telegramdesktop/libtgvoip/archive/${LIBTGVOIP_COMMIT}.tar.gz -> ${PN}-${PV}.tar.gz
  http://${BASE_URI}/files/libtgvoip-2.4.4_p20240706-fix-missing-cstdint-for-gcc-15.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+dsp +alsa -pulseaudio -static-libs +shared -doc -nopie (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-${LIBTGVOIP_COMMIT}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-2.4.4_p20240706-fix-missing-cstdint-for-gcc-15.patch"

  # Will be controlled by us
  sed -e '/^CFLAGS += -DTGVOIP_NO_DSP/d' -i Makefile.am || die
  # https://bugs.gentoo.org/717210
  echo 'libtgvoip_la_LIBTOOLFLAGS = --tag=CXX' >> Makefile.am || die

  test -x "/bin/perl" && autoreconf --install

  # FIX: set by <--disable-dsp>, WebRTC is linked from tg_owt
  ./configure \
    --prefix="/usr" \
    --exec-prefix="${EPREFIX%/}" \
  ...
  # BUG: Error relocating libx32/libtgvoip.so.2.4.4: main: symbol not found
  # FIX: remove from build flags it <-no-pie>
  ldd "$(get_libdir)"/${PN}.so.${SYMVER} || die "library deps work... error"
