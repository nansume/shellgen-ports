...
PVR="${PV}.r461"
SRC_URI="
  https://github.com/ShiftMediaProject/modplug/archive/${PVR}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/media-libs/libmodplug/files/libmodplug-0.8.9.1-timidity.patch
  http://data.gpo.zugaina.org/gentoo/media-libs/libmodplug/files/libmodplug-0.8.9.0-no-fast-math.patch
"
USE_BUILD_ROOT="0"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN#lib}-${PVR}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-0.8.9.1-timidity.patch"
  gpatch -p1 -E < "${FILESDIR}/${PN}-0.8.9.0-no-fast-math.patch"
