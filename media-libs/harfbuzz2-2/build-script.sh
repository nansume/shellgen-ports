...
# Date: 2025-05-19 11:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/harfbuzz/harfbuzz-11.2.1.ebuild
...
PV="11.2.1"
SLOT="6"  # slot=0/6.0.0
SRC_URI="https://github.com/harfbuzz/harfbuzz/releases/download/${PV}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -D wasm=disabled \
    -D cairo=$(usex 'cairo' enabled disabled) \
    -D glib=$(usex 'glib' enabled disabled) \
    -D graphite2=$(usex 'graphite' enabled disabled) \
    -D icu=$(usex 'icu' enabled disabled) \
    -D gobject=$(usex 'introspection' enabled disabled) \
    -D freetype=$(usex 'truetype' enabled disabled) \
    -D introspection=$(usex 'introspection' enabled disabled) \
    -D experimental_api=$(usex 'experimental' true false) \
