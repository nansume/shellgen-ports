#!/bin/sh
# Copyright (C) 2024 Artjom Slepnjov, Shellgen
# License GPLv3: GNU GPL version 3 only
# http://www.gnu.org/licenses/gpl-3.0.html
# Date: 2024-07-26 11:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl +stest +strip +x32

export XPN PF PV WORKDIR BUILD_DIR PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export CC PKG_CONFIG PKG_CONFIG_LIBDIR PKG_CONFIG_PATH

DESCRIPTION="Core libraries for Xine movie player"
HOMEPAGE="https://xine.sourceforge.net/"
LICENSE="GPL-2"
IFS="$(printf '\n\t')"
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
LC_ALL="C"
CATEGORY="${CATEGORY:-${11:?required <CATEGORY>}}"
PN="${PN:-${12:?required <PN>}}"
PN=${PN%%_*}
XPN=${XPN:-$PN}
PV="1.2.13"
SRC_URI="https://downloads.sourceforge.net/xine/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
BUILD_CHROOT=${BUILD_CHROOT:-0}
PDIR=$(pkg-rootdir)
DPREFIX="/usr"
INCDIR="${DPREFIX}/include"
INSTALL_OPTS="install"
HOSTNAME="localhost"
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-static-libs +shared -doc (+musl) +stest +strip -a52 +aac -aalib +alsa -bluray"
IUSE="${IUSE} -cpu_flags_ppc_altivec -css -dav1d -dts -dvb -dxr3 +fbcon +flac -gtk +imagemagick"
IUSE="${IUSE} -jack +jpeg -libcaca -mad +mmap -mng -modplug -musepack -nfs -opengl -oss"
IUSE="${IUSE} -pulseaudio -samba +sftp +sdl -speex -theora -truetype -v4l -vaapi -vcd -vdpau"
IUSE="${IUSE} -vdr -vidix -vis +vorbis +vpx -wavpack -wayland +X -xinerama +xv -xvmc -nls"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
EPREFIX=${SPREFIX}
P="${P:-${XPWD##*/}}"
SN=${P}
PORTS_DIR=${PWD%/$P}
DISTDIR="/usr/distfiles"
DISTSOURCE="${PDIR%/}/sources"
FILESDIR=${DISTSOURCE}
INSTALL_DIR="${PDIR%/}/install"
ED=${INSTALL_DIR}
SDIR="${PDIR%/}/${SRC_DIR}"
PF=$(pfname 'src_uri.lst' "${SRC_URI}")
PKGNAME=${PN}
ZCOMP="unxz"
WORKDIR="${PDIR%/}/${SRC_DIR}"
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
PKG_CONFIG_LIBDIR="/${LIB_DIR}/pkgconfig"
PKG_CONFIG_PATH="${PKG_CONFIG_LIBDIR}:/lib/pkgconfig:/usr/share/pkgconfig"
ABI_BUILD="${ABI_BUILD:-${1:?}}"
BUILD_CHROOT="${7:-${BUILD_CHROOT:?}}"
USE_BUILD_ROOT=${9:-$USE_BUILD_ROOT}

if test "X${USER}" != 'Xroot'; then
  mksrc-prepare
elif test "${BUILD_CHROOT:=0}" -eq '0'; then
  PATH="${PATH:+${PATH}:}${PDIR}/misc.d:${PDIR}/etools.d"
elif test "${BUILD_CHROOT:=0}" -ne '0'; then
  PATH="$(xpath):${PDIR%/}/misc.d:${PDIR%/}/etools.d"
  printf %s\\n "PATH='${PATH}'" "PDIR='${PDIR}'"
fi

. "${PDIR%/}/etools.d/"build-functions

chroot-build || die "Failed chroot... error"

pkginst \
  "#app-arch/xz" \
  "dev-lang/perl  # required for autotools" \
  "#dev-libs/expat  # deps opengl" \
  "#dev-libs/glib  # gtk2?" \
  "dev-libs/gmp  # for gnutls" \
  "#dev-libs/libffi  # for glib" \
  "dev-libs/libtasn1  # for gnutls" \
  "dev-libs/libunistring  # for gnutls" \
  "dev-libs/nettle  # for gnutls" \
  "#dev-libs/pcre  # optional (internal pcre glib-2.68.4) gtk2?" \
  "dev-util/pkgconf" \
  "media-gfx/imagemagick  # imagemagick?" \
  "#media-libs/a52dec  # a52?" \
  "#media-libs/aalib  # aalib?" \
  "media-libs/alsa-lib  # alsa?" \
  "#media-libs/glu  # opengl?" \
  "#media-libs/faad2  # aac?" \
  "media-libs/flac  # flac?" \
  "#media-libs/fontconfig  # truetype?" \
  "#media-libs/freetype  # truetype?" \
  "#media-libs/libcaca  # libcaca?" \
  "media-libs/libjpeg-turbo  # jpeg?" \
  "media-libs/libogg  # speex? theora? vorbis?" \
  "#media-libs/libmad  # mad?" \
  "#media-libs/libmng  # mng?" \
  "#media-libs/libmodplug  # modplug?" \
  "#media-libs/libv4l  # v4l?" \
  "#media-libs/libva  # vaapi?" \
  "media-libs/libvorbis  # vorbis?" \
  "media-libs/libvpx  # vpx?" \
  "#media-libs/libtheora  # theora?" \
  "#media-libs/mesa  # opengl?" \
  "media-libs/sdl  # sdl?" \
  "#media-sound/musepack-tools  # musepack?" \
  "#media-sound/wavpack  # wavpack?" \
  "media-video/ffmpeg" \
  "#net-fs/libnfs  # nfs?" \
  "net-libs/gnutls" \
  "net-libs/libssh2  # sftp?" \
  "net-libs/mbedtls  # deps libssh2" \
  "sys-apps/file" \
  "sys-devel/binutils" \
  "sys-devel/gcc" \
  "#sys-devel/gettext  # for nls (optional)" \
  "#sys-devel/libtool  # (dev-build/libtool) optional" \
  "sys-devel/make" \
  "sys-kernel/linux-headers-musl  # required for: <oss>,<v4l> ?" \
  "sys-libs/musl" \
  "sys-libs/zlib" \
  "x11-base/xorg-proto  # for x11" \
  "#x11-libs/gdk-pixbuf  # gtk2?" \
  "#x11-libs/libdrm  # opengl?" \
  "#x11-libs/libpciaccess  # opengl?" \
  "#x11-libs/libvdpau  # vdpau? opengl?" \
  "x11-libs/libx11  # for x11" \
  "x11-libs/libxau  # for x11" \
  "x11-libs/libxcb  # for x11" \
  "#x11-libs/libxdamage  # opengl?" \
  "x11-libs/libxdmcp  # for x11" \
  "x11-libs/libxext  # required for x11" \
  "x11-libs/libxfixes" \
  "#x11-libs/libxinerama  # xinerama?" \
  "x11-libs/libxrandr" \
  "x11-libs/libxrender  # for libxft" \
  "#x11-libs/libxshmfence  # opengl?" \
  "x11-libs/libxt  # X?" \
  "x11-libs/libxv  # xv?" \
  "#x11-libs/libxvmc  # xvmc?" \
  "#x11-libs/libxxf86vm  # opengl?" \
  || die "Failed install build pkg depend... error"

build-deps-fixfind

. "${PDIR%/}/etools.d/"ldpath-apply
. "${PDIR%/}/etools.d/"path-tools-apply

netuser-fetch "${SRC_URI}" || die "Failed fetch sources... error"
sw-user || die "Failed package build from user... error"  # only for user-build

if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  exit  # only for user-build
elif test "X${USER}" != 'Xroot'; then  # only for user-build
  renice -n '19' -u ${USER}

  cd "${FILESDIR}/" || die "distsource dir: not found... error"

  ${ZCOMP} -dc "${PF}" | tar -C "${PDIR%/}/${SRC_DIR}/" -xkf - || exit &&
  printf %s\\n "${ZCOMP} -dc ${PF} | tar -C ${PDIR%/}/${SRC_DIR}/ -xkf -"

  case $(tc-abi-build) in
    'x32')   append-flags -mx32 -msse2 ;;
    'x86')   append-flags -m32         ;;
    'amd64') append-flags -m64 -msse2  ;;
  esac
  if use 'static-libs'; then
    append-flags -Os
    append-ldflags -Wl,--gc-sections
    append-cflags -ffunction-sections -fdata-sections
  else
    append-flags -O2
  fi
  append-flags -fno-stack-protector -no-pie -g0 -march=$(arch | sed 's/_/-/')

  CC="cc"

  use 'strip' && INSTALL_OPTS="install-strip"

  cd "${BUILD_DIR}/" || die "builddir: not found... error"

  ./configure \
    --prefix="/usr" \
    --exec-prefix="${EPREFIX%/}" \
    --bindir="${EPREFIX%/}/bin" \
    --sysconfdir="${EPREFIX%/}"/etc \
    --libdir="${EPREFIX%/}/$(get_libdir)" \
    --includedir="${INCDIR}" \
    --datadir="${EPREFIX%/}"/usr/share \
    --mandir="${EPREFIX%/}"/usr/share/man \
    --docdir="${EPREFIX%/}"/usr/share/doc/${PN}-${PV} \
    --host=$(tc-chost | sed '/-musl/ s/-/-pc-/;s/musl/gnu/') \
    --build=$(tc-chost | sed '/-musl/ s/-/-pc-/;s/musl/gnu/') \
    --disable-directfb \
    --enable-fb \
    --disable-gnomevfs \
    --enable-optimizations \
    --disable-real-codecs \
    --disable-v4l \
    --disable-w32dll \
    --enable-avformat \
    --enable-ipv6 \
    --with-gnutls \
    --disable-postproc \
    --with-real-codecs-path="/$(get_libdir)/codecs" \
    --with-w32-path="/$(get_libdir)/win32" \
    --with-xv-path="/$(get_libdir)" \
    --without-esound \
    --without-fusionsound \
    --without-libaom \
    --disable-a52dec \
    $(use_enable 'aac' faad) \
    --disable-aalib \
    --disable-bluray \
    --disable-dts \
    --disable-dvb \
    --disable-dxr3 \
    --disable-gdkpixbuf \
    $(use_enable 'jpeg' libjpeg) \
    --disable-mad \
    $(use_enable 'mmap') \
    --disable-mng \
    --disable-modplug \
    --disable-musepack \
    --disable-nfs \
    --disable-opengl \
    --disable-glu \
    --disable-oss \
    --disable-sndio \
    --disable-samba \
    $(use_enable 'sftp') \
    --disable-libv4l \
    --disable-v4l2 \
    --disable-vaapi \
    --disable-vdpau \
    --disable-vis \
    --disable-vidix \
    --disable-xinerama \
    --disable-xvmc \
    --disable-vcd \
    --disable-vdr \
    $(use_enable 'vpx') \
    --disable-wayland \
    $(use_with 'alsa') \
    --without-dav1d \
    $(use_with 'flac' libflac) \
    $(use_with 'imagemagick') \
    --without-jack \
    --without-caca \
    --without-pulseaudio \
    $(use_with 'sdl') \
    --without-speex \
    --without-theora \
    --without-fontconfig \
    --without-freetype \
    $(use_with 'vorbis') \
    --without-wavpack \
    $(use_with X 'x') \
    $(use_with X 'xcb') \
    $(use_enable 'shared') \
    $(use_enable 'static-libs' static) \
    --disable-nls \
    --disable-rpath \
    || die "configure... error"

  make -j "$(nproc)" || die "Failed make build"

  make DESTDIR="${ED}" ${INSTALL_OPTS} || : die "make install... error"

  cd "${ED}/" || die "install dir: not found... error"

  find "$(get_libdir)/" -name '*.la' -delete || die

  exit 0  # only for user-build
fi

cd "${ED}/" || die "install dir: not found... error"

pkg-perm

INST_ABI="$(tc-abi-build)" PN=${XPN} pkg-create-cgz
