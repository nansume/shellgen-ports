...
# https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=directfb2-eglgbm-git

export XPN PF PV WORKDIR BUILD_DIR PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export CC CXX PKG_CONFIG PKG_CONFIG_LIBDIR PKG_CONFIG_PATH

DESCRIPTION="EGL system module for DirectFB2 and supported on GBM (Generic Buffer Manager) platforms."
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/directfb-system-eglgbm.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/directfb-system-eglgbm.pc || die

  ldd "$(get_libdir)"/directfb-2.0-0/systems/libdirectfb_eglgbm.so || die "library deps work... error"
