...
# Date: 2025-04-29 16:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/media-libs/clutter-gtk/clutter-gtk-1.8.4-r2.ebuild
...
pkginst \
  ...
  "x11-libs/libxft  # deps gtk3" \
  "x11-libs/libxi  # deps gtk3" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  ldd "$(get_libdir)"/lib${PN}-1.0.so || die "library deps work... error"
