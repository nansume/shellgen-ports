...
# Date: 2023-12-19 21:00 UTC - last change
...
SRC_URI="http://dl.exactcode.de/oss/${PN}/${PN}-${PV}.tar.gz"
LICENSE="GPL-2"
USER=${USER:-root}
USE_BUILD_ROOT='0'
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  make -j "$(use diet && printf 1 || printf $(cpun) )" \
    ...
    DESTDIR="${INSTALL_DIR}" \
    DIET="" ${PN} install || die "Failed make build or install... error"
  ...
  LD_LIBRARY_PATH= printf Failed\\n | bin/${PN} 's/Failed/Successful... ok/'
