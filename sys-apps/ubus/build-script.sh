...
# Description: OpenWrt RPC daemon and client library
# Homepage: https://git.openwrt.org/project/ubus.git
# License: LGPL-2.1
# Depends: libubox json-c lua51
# Date: 2026-01-22 19:00 UTC - last change
# Build with useflag: +static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://github.com/openwrt/openwrt/archive/master.tar.gz  package/system/ubus/Makefile
...
PV="20260122"
PV="2025.12.02"
HASH="3cc98db1a422dcf560f2d6347fd410f17565a89d"  # 2025.12.02
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  rm -r -- "${BUILD_DIR}"/build/ "${ED}"/usr/bin/ "${ED}"/usr/sbin/
  ...
  mv -v -n "usr/bin" .
  mv -v -n "usr/sbin" .
  if [ -d "$(get_libdir)" ]; then
    mv -v -n "usr/lib/"* -t "$(get_libdir)/"
    rmdir -- "usr/lib/"
  else
