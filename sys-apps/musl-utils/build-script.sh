...
# Date: 2024-08-09 12:00 UTC - last change
...
XPN=${XPN:-$PN}
PV="1.1.12-r7"
SRC_URI="https://github.com/boltlinux/musl-utils/archive/1.1.12-r7.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  PROGS="getconf getent iconv"
  SRCDIR="src"

  cd "${BUILD_DIR}/" || die "builddir: not found... error"

  for I in ${PROGS}; do
    ${CC} ${CPPFLAGS} ${CFLAGS} ${LDFLAGS} "${SRCDIR}"/${I}.c -o ${I} || die "Failed make build"
  done
  ...
  use 'stest' && { bin/getent || : die "binary work... error";}
  ldd "bin/getent" || { use 'static' && true || die "library deps work... error";}
