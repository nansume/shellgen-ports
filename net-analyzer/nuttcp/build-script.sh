...
# Date: 2024-02-27 01:00 UTC - last change
...
SRC_URI="
  http://nuttcp.net/${PN}/${PN}-${PV}.tar.bz2
  http://data.gpo.zugaina.org/gentoo/${CATEGORY}/${PN}/files/${PN}-8.1.4-makefile.patch
  http://data.gpo.zugaina.org/nest/${CATEGORY}/${PN}/files/${PN}-${PV}-ipv6.patch
"
LICENSE="GPL-2"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
if use 'diet'; then
  ...
  if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
    cp -nl usr/include/linux/sockios.h opt/diet/include/linux/
