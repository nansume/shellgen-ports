...
# http://data.gpo.zugaina.org/gentoo/sys-fs/mergerfs/mergerfs-2.40.2.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-xattr +static -shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
EOF
  ...
  use 'xattr' || sed -e 's%USE_XATTR = 1%USE_XATTR = 0%g' -i Makefile || die

  make -j "$(nproc)" || die "Failed make build"

  mkdir -m 0755 -- "${ED}/bin/"
  mv -n build/${PN} -t "${ED}/bin/" || die "make install... error"
  printf %s\\n "Install: ${PN}... ok"
