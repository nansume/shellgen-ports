...
# Date: 2025-03-16 09:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/sys-fs/fuse/fuse-3.16.2.ebuild
# https://git.alpinelinux.org/aports/plain/main/fuse3/APKBUILD
...
PV="3.16.2"
SLOT="${PV%${PV#?}}"
SRC_URI="https://github.com/libfuse/libfuse/releases/download/${PN}-${PV}/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-suid -test -static +static-libs +shared +nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dlibexecdir="/usr/include" \
    -Dwrap_mode="nodownload" \
    -Dbuildtype="release" \
    -Duseroot=false \
    -Dinitscriptdir=" " \
    -Dudevrulesdir="/etc/rules.d" \
