...
# Date: 2024-09-06 13:00 UTC - last change
...
PV="0.7.2"
SRC_URI="
  https://bitbucket.org/agalanin/${PN}/downloads/${PN}-${PV}.tar.gz
  https://libzip.org/download/libzip-1.10.1.tar.xz
  http://data.gpo.zugaina.org/gentoo/sys-fs/${PN}/files/${PN}-0.4.5-makefile.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  mkdir -pm 0755 -- "${WORKDIR}/libzip-1.10.1/build/"
  cd "${WORKDIR}/libzip-1.10.1/build/" || die "builddir: not found... error"

  CPPFLAGS="${CPPFLAGS} -D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE" \
  cmake \
  ...
  make DESTDIR="${BUILD_DIR}/libzip" install || die "make install... error"

  CFLAGS="${CFLAGS} -I${BUILD_DIR}/libzip/${INCDIR#/}"
  CXXFLAGS="${CXXFLAGS} -I${BUILD_DIR}/libzip/${INCDIR#/}"
  LDFLAGS="${LDFLAGS} -L${BUILD_DIR}/libzip/$(get_libdir) -lzip"
  PKG_CONFIG_PATH="${PKG_CONFIG_PATH}:${BUILD_DIR}/libzip/${LIB_DIR}/pkgconfig"
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-0.4.5-makefile.patch"

  make -j "$(nproc)" CXX="${CXX}" CXXFLAGS="${CXXFLAGS} ${LDFLAGS}" || die "Failed make build"

  make DESTDIR="${ED}" prefix="/" ${INSTALL_OPTS} || die "make install... error"
