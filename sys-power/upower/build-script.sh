...
# Date: 2024-10-28 19:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://git.alpinelinux.org/aports/plain/community/upower/APKBUILD
# http://data.gpo.zugaina.org/gentoo/sys-power/upower/upower-1.90.7.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-doc +introspection -ios -selinux -test +shared -nopie (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-lang/python38  # for meson" \
  "dev-libs/expat  # python bundled" \
  "dev-libs/glib74" \
  "dev-libs/gobject-introspection  # optional" \
  "dev-libs/libffi  # for python,glib" \
  "dev-libs/libgudev  # Error: found: NO" \
  "dev-libs/libxslt" \
  ...
  "dev-util/cmake  # it optional? (with meson)" \
  "dev-util/pkgconf" \
  "sys-apps/dbus" \
  ...
  "sys-libs/musl0" \
  "sys-libs/zlib  # deps: setuptools,zipp[meson],glib" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dbuildtype="release" \
    -Dgtk-doc=$(usex 'gtk-doc' true false) \
    -Dman="false" \
  ...
  rm -r -- usr/libexec/upower/ usr/share/installed-tests/

  # fix: meson wrong the pkgconfig
  grep '${prefix}' < $(get_libdir)/pkgconfig/${PN}-glib.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/${PN}-glib.pc || : die
