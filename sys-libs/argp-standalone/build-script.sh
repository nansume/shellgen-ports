...
# Date: 2024-04-02 23:00 UTC - last change
# Build with useflag: +static-libs +shared +musl +x32
...
SRC_URI="
  https://github.com/${PN}/${PN}/archive/refs/tags/${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/sys-libs/${PN}/files/${PN}-${PV}-shared.patch
"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+static-libs +shared (+musl) (-test) +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-${PV}-shared.patch"
  ...
  mkdir -m 0755 "${ED}/usr/" "${ED}/usr/include/"
  mv -vn "argp.h" "${ED}/usr/include/"
