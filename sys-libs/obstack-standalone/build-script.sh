...
# Date: 2024-04-03 09:00 UTC - last change
...
SRC_URI="https://github.com/void-linux/musl-obstack/archive/refs/tags/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
LICENSE="GPL-2"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
PKGNAME=${PN}
ZCOMP="gunzip"
WORKDIR="${PDIR%/}/${SRC_DIR}/musl-obstack-${PV}"
S="${PDIR%/}/${SRC_DIR}/musl-obstack-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cd "${ED}/$(get_libdir)/pkgconfig/"
  mv -vn musl-obstack.pc obstack-standalone.pc || die
