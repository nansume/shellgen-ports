...
# Date: 2024-07-18 12:00 UTC - last change
...
PN=${PN%[0-9]*}
PV="1.11"
SRC_URI="mirror://gnu/gdbm/${PN}-${PV}.tar.gz"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+berkdb -exporter -nls -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc$(usex static ' --static')" AR="ar"
  ...
  use 'static-libs' || find "${ED}/$(get_libdir)/" -name '*.la' -delete
  mv -n "${ED}"/usr/include/gdbm/gdbm.h -t "${ED}"/usr/include/ || die
