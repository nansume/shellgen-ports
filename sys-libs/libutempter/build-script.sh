...
# Date: 2025-05-02 17:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/sys-libs/libutempter/libutempter-1.2.1.ebuild
...
XPN=${XPN:-$PN}
PV="1.2.1"
SRC_URI="ftp://ftp.altlinux.org/pub/people/ldv/${PN}/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed \
    -e "/^libdir /s:/usr/lib:${EPREFIX%/}/$(get_libdir):" \
  ...
  if ! use 'static-libs'; then
    sed -e '/^STATICLIB/d' -e '/INSTALL.*STATICLIB/d' -i Makefile
  fi
  ...
  ln -s ../$(get_libdir)/misc/utempter/${PROG} "${ED}"/sbin/

  cd "${ED}/" || die "install dir: not found... error"

  use 'doc' || rm -r -- "usr/share/man/" "usr/share/"

  strip --verbose --strip-all "$(get_libdir)/misc/utempter/"${PROG}
