...
# http://data.gpo.zugaina.org/gentoo/dev-lang/python/python-3.10.17-r1.ebuild
...
if pkg-is "dev-lang/python3-10"; then
  pkginst "dev-lang/python3-10"
elif pkg-is "dev-lang/python3-8"; then
  pkginst "dev-lang/python3-8"
fi
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e "s:@@GENTOO_LIBDIR@@:$(get_libdir):g" -i setup.py || die

  # force the correct number of jobs
  # https://bugs.gentoo.org/737660
  sed -e "s:-j0:-j$(nproc):" -i Makefile.pre.in || : die
  sed -e "/self\.parallel/s:True:$(nproc):" -i setup.py || die
  ...
  ./configure \
    ...
    $(test -n "${CFLAGS}" && printf CFLAGS="${CFLAGS}") \
  ...
  ln -vsf ../../lib/python${PYVER} "$(get_libdir)/"
