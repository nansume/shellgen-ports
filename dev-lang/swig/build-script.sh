...
# Date: 2024-07-19 13:00 UTC - last change
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-ccache -doc -pcre -test +static (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  "#dev-libs/pcre2  # useflag <pcre>" \
  "dev-util/pkgconf" \
  "sys-devel/binutils" \
  "sys-devel/gcc" \
  "sys-devel/make" \
  "sys-libs/musl" \
  "#sys-libs/zlib  # useflag <ccache>" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  use 'stest' && { bin/${PN} -help -version || : die "binary work... error";}
