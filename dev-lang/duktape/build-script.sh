...
# Date: 2023-11-28 22:00 UTC - last change
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  #src-patch "${DISTSOURCE}/${PN}-${PV}-musl-x32.diff"
  ...
  printf %s\\n "FCFLAGS='${FCFLAGS}'" "FFLAGS='${FFLAGS}'" "LDFLAGS='${LDFLAGS}'"
	...
	mv Makefile.sharedlibrary Makefile || printf %s\\n "failed to rename makefile" >&2

  . runverb \
  make \
  	DESTDIR="${INSTALL_DIR}" \
    INSTALL_PREFIX="${SPREFIX%/}" \
  ...
  cd "${INSTALL_DIR}/" || exit
	...
	mkdir -m 0755 "${DPREFIX#/}/"
	mv -n include "${DPREFIX#/}/"

	sed -i -e "/^includedir=/ s|\${prefix}|${DPREFIX}|" "$(get_libdir)/pkgconfig/${PN}.pc"
