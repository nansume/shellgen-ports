...
# Date: 2024-06-24 16:00 UTC - last change
# Build with useflag:
...
PV="5.1.5"
XPV=${PV%.*}
SRC_URI="https://dev.gentoo.org/~soap/distfiles/${PN}-${PV}.tar.xz"
USER=${USER:-root}
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+static +static-libs +shared +musl (-test) +strip"
IUSE="${IUSE} (-man) -doc +deprecated -readline"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  if use 'static'; then
    sed 's:$(LINK) $(lua5_1_OBJECTS):$(LINK) -s -static --static $(lua5_1_OBJECTS):' -i Makefile
    sed 's:$(LINK) $(luac5_1_OBJECTS):$(LINK) -s -static --static $(luac5_1_OBJECTS):' -i Makefile
  fi
