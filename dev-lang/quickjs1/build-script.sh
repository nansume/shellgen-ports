...
XPN=${XPN:-$PN}
PN=${PN%[0-9]}
PV="2025-09-13-2"  # no build
PV="2024-01-13"
XPV=${PV//-/.}  # no-posix
SRC_URI="https://bellard.org/quickjs/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+static +static-libs -shared -doc (+musl) +stest +strip -lto"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed \
    ...
    -e '/$(STRIP) .*/d' \
    -e "s|/lib/|/$(get_libdir)/|" \
    -e "s|\$(PREFIX)/include/|/usr/include/|" \
    -i Makefile || die "Failed to change Makefile"
