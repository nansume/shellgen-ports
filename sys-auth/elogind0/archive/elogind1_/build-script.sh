...
# BUG: ModuleNotFoundError: No module named jinja2. Build Failed.
# BUG: /src/basic/linux/if.h:216:3: error: unknown type name 'x25_hdlc_proto
...
SRC_URI="
  ...
  http://data.gpo.zugaina.org/gentoo/sys-auth/elogind/files/${PN}-252.9-nodocs.patch
  http://data.gpo.zugaina.org/gentoo/sys-auth/elogind/files/${PN}-${PV}-revert-s2idle.patch
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-252.9-nodocs.patch"
  # See also:
  # https://github.com/elogind/elogind/issues/285
  patch -p1 -E < "${FILESDIR}/${PN}-${PV}-revert-s2idle.patch"  #bug 939042
  ...
  sed -e "s/#RemoveIPC=yes/RemoveIPC=no/" -i src/login/logind.conf.in || die
