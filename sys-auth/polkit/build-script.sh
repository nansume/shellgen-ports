#!/bin/sh
# Maintainer: Artjom Slepnjov <shellgen-at-uncensored-dot-citadel-dot-org>
# Date: 2025-05-31 15:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/sys-auth/polkit/polkit-126-r1.ebuild

export XPN PF PV WORKDIR BUILD_DIR PKGNAME BUILD_CHROOT LC_ALL BUILD_USER SRC_DIR IUSE SRC_URI SDIR
export XABI SPREFIX EPREFIX DPREFIX PDIR P SN PN PORTS_DIR DISTDIR DISTSOURCE FILESDIR INSTALL_DIR ED
export CC CXX PKG_CONFIG PKG_CONFIG_LIBDIR PKG_CONFIG_PATH

DESCRIPTION="Policy framework for controlling privileges for system-wide services"
HOMEPAGE="https://www.freedesktop.org/wiki/Software/polkit https://github.com/polkit-org/polkit"
LICENSE="LGPL-2"
IFS="$(printf '\n\t')"
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
LC_ALL="C"
CATEGORY="${CATEGORY:-${11:?required <CATEGORY>}}"
PN="${PN:-${12:?required <PN>}}"
PN=${PN%%_*}
XPN=${XPN:-$PN}
PV="126"
SRC_URI="
  https://github.com/polkit-org/polkit/archive/${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/sys-auth/polkit/files/${PN}-${PV}-elogind.patch
  http://data.gpo.zugaina.org/gentoo/sys-auth/polkit/files/${PN}-${PV}-realpath.patch
  http://data.gpo.zugaina.org/gentoo/sys-auth/polkit/files/${PN}-${PV}-musl.patch
"
USE_BUILD_ROOT="0"
BUILD_CHROOT=${BUILD_CHROOT:-0}
PDIR=$(pkg-rootdir)
DPREFIX="/usr"
INCDIR="${DPREFIX}/include"
HOSTNAME="localhost"
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-examples -gtk +introspection -kde -pam -nls -selinux -systemd -test"
IUSE="${IUSE} -static-libs +shared -nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
EPREFIX=${SPREFIX}
P="${P:-${XPWD##*/}}"
SN=${P}
PORTS_DIR=${PWD%/$P}
DISTDIR="/usr/distfiles"
DISTSOURCE="${PDIR%/}/sources"
FILESDIR=${DISTSOURCE}
INSTALL_DIR="${PDIR%/}/install"
ED=${INSTALL_DIR}
SDIR="${PDIR%/}/${SRC_DIR}"
PF=$(pfname 'src_uri.lst' "${SRC_URI}")
PKGNAME=${PN}
ZCOMP="gunzip"
WORKDIR="${PDIR%/}/${SRC_DIR}"
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${PN}-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
PKG_CONFIG_LIBDIR="/${LIB_DIR}/pkgconfig"
PKG_CONFIG_PATH="${PKG_CONFIG_LIBDIR}:/lib/pkgconfig:/usr/share/pkgconfig"
ABI_BUILD="${ABI_BUILD:-${1:?}}"
BUILD_CHROOT="${7:-${BUILD_CHROOT:?}}"
USE_BUILD_ROOT=${9:-$USE_BUILD_ROOT}
PROG="pkcheck"

if test "X${USER}" != 'Xroot'; then
  mksrc-prepare
elif test "${BUILD_CHROOT:=0}" -eq '0'; then
  PATH="${PATH:+${PATH}:}${PDIR}/misc.d:${PDIR}/etools.d"
elif test "${BUILD_CHROOT:=0}" -ne '0'; then
  PATH="$(xpath):${PDIR%/}/misc.d:${PDIR%/}/etools.d"
  printf %s\\n "PATH='${PATH}'" "PDIR='${PDIR}'"
fi

. "${PDIR%/}/etools.d/"build-functions

chroot-build || die "Failed chroot... error"

pkginst \
  "app-text/docbook-xml-dtd:4.1.2" \
  "app-text/docbook-xsl-stylesheets" \
  "#dev-build/cmake3  # it optional?" \
  "dev-build/meson7  # build tool" \
  "#dev-build/muon  # alternative for meson" \
  "dev-build/samurai  # alternative for ninja" \
  "dev-lang/duktape" \
  "dev-lang/perl" \
  "dev-lang/python3-8  # deps meson" \
  "dev-libs/expat  # deps meson,python" \
  "dev-libs/glib74" \
  "dev-libs/gobject-introspection74" \
  "dev-libs/libffi  # deps meson" \
  "dev-libs/libxslt" \
  "dev-libs/pcre2  # deps glib74" \
  "dev-util/pkgconf" \
  "sys-apps/dbus  # required, deps elogind" \
  "sys-apps/kmod  # deps eudev" \
  "sys-auth/elogind  # !systemd?" \
  "sys-devel/binutils6" \
  "sys-devel/gcc6" \
  "sys-devel/gettext-tiny  # required" \
  "sys-devel/m4  # required for ninja" \
  "sys-devel/make" \
  "sys-fs/eudev  # deps elogind" \
  "sys-libs/libcap  # deps elogind" \
  "sys-libs/musl" \
  "sys-libs/zlib  # deps meson" \
  "x11-libs/libx11  # deps dbus" \
  "x11-libs/libxau  # deps dbus" \
  "x11-libs/libxcb  # deps dbus" \
  "x11-libs/libxdmcp  # deps dbus" \
  "x11-libs/libxt  # deps dbus" \
  || die "Failed install build pkg depend... error"

build-deps-fixfind

. "${PDIR%/}/etools.d/"ldpath-apply
. "${PDIR%/}/etools.d/"path-tools-apply

netuser-fetch "${SRC_URI}" || die "Failed fetch sources... error"
sw-user || die "Failed package build from user... error"  # only for user-build

if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  exit  # only for user-build
elif test "X${USER}" != 'Xroot'; then  # only for user-build
  renice -n '19' -u ${USER}

  . "${PDIR%/}/etools.d/"epython

  cd "${FILESDIR}/" || die "distsource dir: not found... error"

  ${ZCOMP} -dc "${PF}" | tar -C "${PDIR%/}/${SRC_DIR}/" -xkf - || exit &&
  printf %s\\n "${ZCOMP} -dc ${PF} | tar -C ${PDIR%/}/${SRC_DIR}/ -xkf -"

  case $(tc-abi-build) in
    'x32')   append-flags -mx32 -msse2            ;;
    'x86')   append-flags -m32 -msse -mfpmath=sse ;;
    'amd64') append-flags -m64 -msse2             ;;
  esac
  append-flags -O2 -fno-stack-protector -g0 -march=$(arch | sed 's/_/-/')

  CC="gcc" CXX="g++"

  cd "${BUILD_DIR}/" || die "builddir: not found... error"

  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-elogind.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-realpath.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-musl.patch

  # bug #401513
  sed -e 's|unix-group:@PRIVILEGED_GROUP@|unix-user:@PRIVILEGED_GROUP@|' -i src/polkitbackend/*-default.rules.in

  meson setup \
    --default-library=$(usex 'shared' both static) \
    -D prefix="/usr" \
    -D bindir="/bin" \
    -D libdir="/$(get_libdir)" \
    -D includedir="/usr/include" \
    -D datadir="/usr/share" \
    -D localstatedir="${EPREFIX%/}"/var \
    -D wrap_mode="nodownload" \
    -D buildtype="release" \
    -D b_colorout="never" \
    -D authfw=$(usex 'pam' pam shadow) \
    -D examples=false \
    -D gtk_doc=false \
    -D os_type="" \
    -D pam_module_dir="/lib/security" \
    -D privileged_group=0 \
    -D session_tracking=$(usex 'systemd' logind elogind) \
    -D systemdsystemunitdir="/lib/systemd/systemd" \
    -D libs-only=false \
    -D introspection=$(usex 'introspection' true false) \
    -D gettext=$(usex 'nls' true false) \
    -D man=$(usex 'man' true false) \
    -D b_pie="false" \
    -D tests=$(usex 'test' true false) \
    -D strip=$(usex 'strip' true false) \
    "${BUILD_DIR}/build" "${BUILD_DIR}" \
    || die "meson setup... error"

  ninja -j "$(nproc)" -C "${BUILD_DIR}/build" || die "Build... Failed"

  DESTDIR="${ED}" meson install --no-rebuild -C "${BUILD_DIR}/build" || die "meson install... error"

  if use 'examples'; then
    docinto examples
    dodoc src/examples/*.c src/examples/*.policy*
  fi

  if [ "${UID}" -eq '0' ]; then
    diropts -m 0700 -o polkitd
  fi
  keepdir /etc/polkit-1/rules.d

  cd "${ED}/" || die "install dir: not found... error"

  grep '${prefix}' < $(get_libdir)/pkgconfig/${PN}-agent-1.pc
  sed \
    -e '/^datadir=/ s@/share@/usr/share@' \
    -e '/^includedir=/ s@/include@/usr/include@' \
    -e 's|${prefix}||' \
    -e 's|${datadir}|/usr/share|' \
    -i $(get_libdir)/pkgconfig/${PN}-*.pc

  mv -v -n "usr/lib/"* -t "lib/"

  rm -v -r -- "lib/systemd/" "usr/lib/" "usr/share/locale/"

  LD_LIBRARY_PATH="${LD_LIBRARY_PATH:+${LD_LIBRARY_PATH}:}${ED}/$(get_libdir)"
  use 'stest' && { bin/${PROG} --version || die "binary work... error";}
  ldd "bin/${PROG}" || { use 'static' && true || die "library deps work... error";}

  exit 0  # only for user-build
fi

cd "${ED}/" || die "install dir: not found... error"

pkg-perm

INST_ABI="$(tc-abi-build)" PN=${XPN} PV=${PV} pkg-create-cgz