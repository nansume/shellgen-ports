...
# Date: 2024-09-12 18:00 UTC - last change
...
PV="1.14.19"
SRC_URI="
  https://github.com/${MY_PN}/${MY_PN}/archive/release-${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/net-libs/libupnp/files/libupnp-1.14.12-disable-network-tests.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-blocking-tcp -debug -doc +reuseaddr -samples -ssl -static-libs +shared -doc (+musl) +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/${MY_PN}-release-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-1.14.12-disable-network-tests.patch"
