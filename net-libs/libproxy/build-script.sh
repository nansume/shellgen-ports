...
# Date: 2024-08-08 17:00 UTC - last change
...
PN=${PN%%_*}
XPN=${XPN:-$PN}
PV="0.5.6"  # requires >= meson-0.59.0
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+duktape -gnome -gtk-doc -introspection -test +vala -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/libxml2  # for vala" \
  "dev-libs/libxslt  # for vala" \
  "dev-libs/openssl3" \
  "dev-libs/pcre  # for vala" \
  ...
  "sys-devel/make" \
  "sys-libs/musl" \
  "sys-libs/zlib  # deps: setuptools,zipp[meson],glib (optional?)" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dtests="false" \
    -Dconfig-env="true" \
    -Dconfig-gnome=$(usex 'gnome' true false) \
    -Dconfig-windows="false" \
    -Dconfig-sysconfig="true" \
    -Dconfig-osx="false" \
    -Dconfig-kde="true" \
    -Dpacrunner-duktape=$(usex 'duktape' true false) \
    -Dvapi="false" \
    -Dcurl=$(usex 'duktape' true false) \
