...
# Date: 2025-05-03 21:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://data.gpo.zugaina.org/gentoo/net-libs/tox/tox-0.2.20.ebuild
...
IUSE="${IUSE} +log-error -log-info -log-trace -log-warn -test"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/c-toxcore-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  rm -v -r -- "usr/share/bash-completion/" "usr/share/"

  # fix: cmake wrong the pkgconfig
  grep '${prefix}' < $(get_libdir)/pkgconfig/toxcore.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/toxcore.pc
