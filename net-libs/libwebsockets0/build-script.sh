...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's/ websockets_shared//g' -i cmake/libwebsockets-config.cmake.in
  sed -e 's/ OR PC_OPENSSL_FOUND//g' -i lib/tls/CMakeLists.txt
  sed -e '/PC_OPENSSL/d' -i lib/tls/CMakeLists.txt
