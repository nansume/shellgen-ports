...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/mbedtls-3.6.2-allow-install-headers-to-different-location.patch"
  patch -p1 -E < "${FILESDIR}/mbedtls-3.6.2-exclude-static-3dparty.patch"
  ...
  ldd "$(get_libdir)/libmbedcrypto.so" || { use 'static' && true || : die "library deps work... error";}
