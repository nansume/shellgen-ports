...
# Date: 2024-01-23 09:00 UTC - last change
# Build with useflag: +x32
...
PV="4.1.1"
DESCRIPTION="(OTR) allows you to have private conversations over instant messaging"
...
IUSE="+static-libs +shared -patch (-musl) -test +strip"
#IUSE="${IUSE} -idn -kerberos -pam"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  bin/otr_mackey -V || true

  ldd "bin/otr_mackey" || die
