...
# Date: 2024-11-26 16:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs -nopie -patch -doc -xstub -diet +musl -stest +strip +x32

# <orig-url-build-script> portage/net-libs/glib-networking/glib-networking-2.58.0.ebuild
...
XPN=${XPN:-$PN}
PV="2.58.0"
SRC_URI="ftp://ftp.fau.de/gnome/sources/${PN}/${PV%.*}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-gnome +libproxy +ssl -test -static-libs +shared -nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  "app-misc/ca-certificates  # optional" \
  ...
  "dev-util/pkgconf" \
  "#gnome-base/gsettings-desktop-schemas  # optional" \
  "net-libs/gnutls" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  rm -r -- "usr/lib/" "usr/share/"  # remove relative with systemd
