...
LICENSE="BSD"  # license non-compatible with gnutls[LGPL2]
IFS="$(printf '\n\t')"
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
...
XPN=${XPN:-$PN}
PV="2.0.10"
SRC_URI="https://github.com/arvidn/libtorrent/releases/download/v${PV}/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+dht -debug -examples -gnutls -python +ssl -test -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/openssl3  # compatible license" \
  "dev-util/cmake" \
  "#net-libs/gnutls" \
  "sys-devel/binutils" \
  "sys-devel/gcc" \
  ...
  "sys-libs/musl" \
  "sys-libs/zlib  # deps cxx-boost,gnutls" \
