...
# Date: 2025-04-03 22:00 UTC - last change
# Build with useflag: -static +static-libs +shared -lfs +nopie +patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/net-libs/libsrtp/libsrtp-2.6.0.ebuild
...
SRC_URI="
  ...
  http://data.gpo.zugaina.org/gentoo/net-libs/${PN}/files/${PN}-2.4.2-doc.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-debug -doc -nss +openssl +static-libs -test +shared +nopie (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/libsrtp-2.4.2-doc.patch
  ...
  meson setup \
    ...
    -Dbuildtype="release" \
    -Dcrypto-library=$(usex 'openssl' openssl nss) \
