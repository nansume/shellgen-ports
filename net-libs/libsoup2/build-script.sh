...
# http://data.gpo.zugaina.org/gentoo/net-libs/libsoup/libsoup-2.74.3.ebuild
...
PV="2.74.3"
SRC_URI="
  https://download.gnome.org/sources/libsoup/${PV%.*}/${PN}-${PV}.tar.xz
  #http://data.gpo.zugaina.org/gentoo/net-libs/libsoup/files/libxml2-2.12.patch
"
USE_BUILD_ROOT="0"
...
IUSE="${IUSE} -vala -static-libs +shared -nopie -doc (+musl) -stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dbrotli=$(usex 'brotli' enabled disabled) \
    ...
    -Dsysprof=$(usex 'sysprof' enabled disabled) \
