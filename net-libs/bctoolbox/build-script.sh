...
# Date: 2025-04-03 20:00 UTC - last change
# Build with useflag: static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://data.gpo.zugaina.org/nest/net-libs/bctoolbox/bctoolbox-5.3.86.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-debug +ssl -test -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e "2s|^prefix=.*|prefix=|;t" -i $(get_libdir)/pkgconfig/${PN}.pc || : die
