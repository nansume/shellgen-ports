...
# Date: 2025-04-04 11:00 UTC - last change
# Build with useflag: static +static-libs/+shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/nest/net-libs/ortp/ortp-5.3.86.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-debug -doc -ntp-timestamp -minimal -test -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # fix version
  sed -e '/project(Ortp/s|0|1|' -i CMakeLists.txt || die "sed failed for CMakeLists.txt"
  ...
  sed \
    ...
    -e "3s|^exec_prefix=/usr/bin|exec_prefix=/usr|;t" \
    -e "s| -L/usr/lib | -L/$(get_libdir) |" \
