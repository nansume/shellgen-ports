...
SRC_URI="
  ...
  http://data.gpo.zugaina.org/gentoo/dev-qt/qtbase/files/qtbase-6.8.0-qcontiguouscache.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  append-flags -Os -fno-stack-protector -march=$(arch | sed 's/_/-/')
  ...
  patch -p1 -E < "${FILESDIR}"/qtbase-6.8.0-qcontiguouscache.patch
  ...
  #sed -e 's|${prefix}|/|g' -i "$(get_libdir)/"pkgconfig/Qt6*.pc
