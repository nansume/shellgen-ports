...
# Date: 2025-05-17 13:00 UTC - last change
# Build with useflag: -static-libs +shared -ssl +glib -python -doc -xstub +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/dev-qt/qtxmlpatterns/qtxmlpatterns-5.15.16.ebuild
...
XPN=${XPN:-$PN}
PN=${PN%[0-9]*}
SPN="qtxmlpatterns-everywhere-opensource-src"
PV="5.15.16"
...
BUILD_DIR="${PDIR%/}/${SRC_DIR}/qtxmlpatterns-everywhere-src-${PV}"
PWD=${PWD%/}; PWD=${PWD:-/}
LIB_DIR=$(get_libdir)
LIBDIR="/${LIB_DIR}"
PKG_CONFIG="pkgconf"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's|${prefix}|/|g' -i "$(get_libdir)/"pkgconfig/Qt5XmlPatterns.pc
  sed -e '/^QMAKE_PRL_BUILD_DIR/d' -i "$(get_libdir)/"libQt5XmlPatterns.prl
