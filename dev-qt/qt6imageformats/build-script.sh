...
# http://data.gpo.zugaina.org/gentoo/dev-qt/qtimageformats/qtimageformats-6.9.0.ebuild
# http://data.gpo.zugaina.org/gentoo/dev-qt/qtimageformats/qtimageformats-6.8.3.ebuild
...
PN=${PN%%_*}
SPN1="qtimageformats-everywhere-opensource-src"
SPN2="qtimageformats-everywhere-src"
XPN=${XPN:-$PN}
PV="6.9.0"
PV="6.5.5"
PV="6.8.3"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-mng -static-libs +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake -B build -G Ninja \
    ...
    -D QT_FEATURE_jasper=OFF \
    -D QT_FEATURE_mng=$(usex 'mng' ON OFF) \
    -D QT_FEATURE_tiff=ON \
    -D QT_FEATURE_webp=ON \
    -D QT_FEATURE_system_tiff=OFF \
    -D QT_FEATURE_system_webp=OFF \
  ...
  ldd "$(get_libdir)"/qt6/plugins/imageformats/libqtga.so || die "library deps work... error"
