...
# Date: 2025-05-22 19:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/dev-qt/qt5compat/qt5compat-6.8.2-r1.ebuild
...
PN=${PN%%_*}
SPN1="qt5compat-everywhere-opensource-src"
SPN2="qt5compat-everywhere-src"
XPN=${XPN:-$PN}
PV="6.5.5"
PV="6.8.3"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/Qt6Core5Compat.pc
  sed -e 's|${prefix}||' -i $(get_libdir)/pkgconfig/Qt6Core5Compat.pc

  ldd "$(get_libdir)"/libQt6Core5Compat.so.${PV} || die "library deps work... error"
