...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  ./configure \
    ...
    --exec-prefix="${EPREFIX%/}" \
    --libdir="${EPREFIX%/}/$(get_libdir)" \
    --with-pkg-config-libdir="/$(get_libdir)/pkgconfig" \
  ...
  ln -s ${BUILD_DIR}/${PN2}/usr/include/ncursesw/* ${BUILD_DIR}/${PN2}/usr/include
  ...
  ./configure \
    ...
    --with-ncurses-libs="$(usex !shared ${BUILD_DIR}/${PN2})/$(get_libdir)" \
    $(use_with 'x') \
