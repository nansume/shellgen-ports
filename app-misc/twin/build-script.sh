...
# Date: 2025-02-25 21:00 UTC - last change
# Build with useflag: -static +shared -unicode -lfs +nopie -patch +doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/bgo-overlay/app-misc/twin/twin-0.6.1-r1.ebuild
...
XPN=${XPN:-$PN}
PV="0.9.0"
SRC_URI="https://github.com/cosmos72/twin/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-ncurses -oldgtk -unicode -X -xml -static +shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "sys-devel/libtool" \
  "sys-devel/make" \
  "sys-kernel/linux-headers-musl  # for gpm" \
  "sys-libs/gpm" \
  "sys-libs/musl0" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  append-cxxflags -fpermissive  # FIX: build... Failed
