...
# Date: 2024-07-31 18:00 UTC - last change
...
PV="3.0.5"
SRC_URI="
  http://ftp2.osuosl.org/pub/blfs/conglomeration/abiword/abiword-3.0.5.tar.gz
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${WORKDIR}"/patches/${PN}-3.0.0-librevenge.patch
  patch -p1 -E < "${WORKDIR}"/patches/${PN}-3.0.2-explicit-python.patch
  patch -p1 -E < "${WORKDIR}"/patches/${PN}-3.0.4-enchant-2.patch # backport
  patch -p1 -E < "${WORKDIR}"/patches/${PN}-3.0.4-pygobject.patch
