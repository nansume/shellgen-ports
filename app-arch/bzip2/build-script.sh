...
# Date: 2023-11-18 21:00 UTC - last change
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -i \
    ...
    -e "/${PN}$/!s:cp -f ${PN} :ln -s -f ${PN} :" \
    -e 's:ln -s -f $(PREFIX)/bin/:ln -s -f :' \
    -e 's:$(PREFIX)/lib:$(PREFIX)/$(LIBDIR):g' \
    Makefile

  make -f 'Makefile-libbz2_so' all || exit
  ...
  cp -vnul "${WORKDIR}/libbz2.so.${PV}" ${LIB_DIR}/
  ln -sf "libbz2.so.${PV}" "${LIB_DIR}/libbz2.so.${PV%.*}"
  ln -sf "libbz2.so.${PV}" "${LIB_DIR}/libbz2.so"
