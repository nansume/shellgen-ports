...
# Date: 2024-09-24 19:00 UTC - last change
...
PROG="dgen"
SRC_URI="
  https://downloads.sourceforge.net/dgen/files/${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/games-emulation/dgen-sdl/files/${PN}-${PV}-joystick.patch
  http://data.gpo.zugaina.org/gentoo/games-emulation/dgen-sdl/files/${PN}-${PV}-AM_PROG_AR.patch
  #http://data.gpo.zugaina.org/gentoo/games-emulation/dgen-sdl/files/${PN}-${PV}-clang-c++11.patch
"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-joystick -opengl -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "media-libs/sdl  # optional: sdl[joystick?,opengl?]" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-joystick.patch
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-AM_PROG_AR.patch
  #patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-clang-c++11.patch
  ...
  make -j "$(nproc)" -C musa m68kops.h || die "Failed make build"
