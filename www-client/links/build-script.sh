...
# Date: 2023-11-16 21:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/www-client/links/links-2.30.ebuild
...
PV="2.29"
PV="2.30"
SRC_URI="http://links.twibright.com/download/${PN}-${PV}.tar.bz2"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-avif -brotli -bzip2 +fbcon +freetype +gpm +jpeg -libevent -livecd -lzip"
IUSE="${IUSE} -lzma -selinux +ssl -suid -svga +tiff +webp -X +zlib -zstd"
IUSE="${IUSE} -xz +ipv6 +static -shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/expat" \
  "dev-libs/gmp" \
  "#dev-libs/openssl3  # for shared" \
  "dev-libs/openssl  # for static" \
  "dev-util/pkgconf" \
  "media-libs/freetype" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  append-flags -ffast-math  #It improves performance of image scaler.
