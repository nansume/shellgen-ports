...
pkginst \
  ...
  "dev-libs/dietlibc0  #dietlibc1 # 0.34-x32 or 0.35-x86" \
  "sys-devel/binutils6" \
  "sys-devel/gcc6" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # BUG: diet libc no full support <pthread.h>
  # usr/include/pthread.h
  # ftp_listener.c: undefined reference to _pthread_cleanup_push
  # ftp_listener.c: undefined reference to _pthread_cleanup_pop
  #use 'diet' && {
  ...
  #sed -e 's|#include <pthread.h>|#include </usr/include/pthread.h>|' -i src/ftp_listener.c
  #sed -e 's|-lpthread ||;s|-pthread ||' -i configure Makefile* src/Makefile*
  #export LIBS="-pthread"

  # error: conflicting types for 'dprintf'
  #sed -e "s|fdprintf|dprintf|" -i src/file_list.c
  ...
  ./configure \
    ...
    #$(usex '_diet' LIBS="${LIBS}") \
    #$(usex '_diet' LDFLAGS="${LDFLAGS}") \
    #|| die "configure... error"
