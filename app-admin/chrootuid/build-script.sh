...
# Date: 2025-12-19 01:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub +diet -musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/app-admin/chrootuid/chrootuid-1.3-r2.ebuild
...
XPN=${XPN:-$PN}
PV="1.3"
SRC_URI="http://ftp.porcupine.org/pub/security/${PN}${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  make -j "$(nproc)" CC="${CC}" CFLAGS="${CFLAGS} ${LDFLAGS}" || die "Failed make build"
