...
# Homepage: https://git.openwrt.org/project/uci.git
# License: LGPL-2.1
# Depends: libubox lua51
# Date: 2026-01-22 16:00 UTC - last change
# Build with useflag: +static +static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://github.com/openwrt/openwrt/archive/master.tar.gz  package/system/uci/Makefile

# BUG: with gcc6 no build
...
PV="20260122"
PV="2025.12.02"
HASH="66127cd76c5d0bd46d5a90302cc6110f53a4e2f8"  # 2025.12.02
...
pkginst \
  ...
  "dev-util/pkgconf" \
  "sys-devel/binutils9" \
  "sys-devel/gcc9  # BUG: with gcc6 no build" \
  "sys-devel/make" \
  "sys-libs/musl" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  mv -v -n "build/"*.a "${ED}/"$(get_libdir)/
  cp -v -r "${PDIR%/}"/files/* -t "${ED}"/

  cd "${ED}/" || die "install dir: not found... error"

  mkdir -m 0755 -- etc/ etc/uci-defaults/
