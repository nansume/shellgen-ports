...
# Homepage: https://git.openwrt.org/project/netifd.git
# License: GPL-2.0
# Depends: libuci libnl-tiny libubus jshn libubox libudebug ucode
# Date: 2026-01-24 01:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://github.com/openwrt/openwrt/archive/master.tar.gz  package/network/config/netifd/Makefile
...
PV="20260122"
PV="2025.10.20"
HASH="777f5942fa7d6245f6ad29daa1daecc400344d37"  # 2025.10.20
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  append-flags -Os
  append-cflags -I/usr/include/libnl-tiny
  ...
  mkdir -m 0755 -- "${ED}"/etc/udhcpc.user.d/
  cp -v "${BUILD_DIR}"/scripts/utils.sh -t "${ED}"/lib/netifd/
  cp -v "${BUILD_DIR}"/scripts/netifd-proto.sh -t "${ED}"/lib/netifd/
  chmod +x "${ED}"/etc/uci-defaults/14_migrate-dhcp-release
  chmod +x "${ED}"/etc/hotplug.d/iface/00-netstate
