...
# Date: 2025-05-02 19:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# BUG: compile against dietlibc: IPV6_V6ONLY undeclared - build failed

# http://data.gpo.zugaina.org/gentoo/net-misc/geomyidae/geomyidae-0.96.ebuild
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-ssl +static -shared -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e '/GEOM_LDFLAGS/s:-L/usr/lib ::' -i Makefile || die 'sed on Makefile failed'

  use 'ssl' || sed -e '/^TLS_CFLAGS =/ s:-DENABLE_TLS::;/${CC} -o/ s:${GEOM_LDFLAGS}::' -i Makefile
