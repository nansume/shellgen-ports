...
# Date: 2025-04-14 09:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/net-misc/wget2/wget2-2.2.0.ebuild
...
LICENSE="GPL-3+ LGPL-3+"  # LGPL for libwget
IFS="$(printf '\n\t')"
XPWD=${XPWD:-$PWD}
XPWD=${5:-$XPWD}
PKG_DIR="/pkg"
...
XPN=${XPN:-$PN}
PV="2.2.0"
SRC_URI="ftp://ftp.gnu.org/gnu/wget/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  ./configure \
    ...
    --with-plugin-support \
    $(usex !gnutls --with-openssl=auto-gpl-compat) \
