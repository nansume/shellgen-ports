...
# Homepage: https://git.openwrt.org/project/rpcd.git
# License: ISC
# Depends: libubox json-c ubus ucode uci libiwinfo
# Date: 2026-01-24 01:00 UTC - last change
# Build with useflag: +static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# https://github.com/openwrt/openwrt/archive/master.tar.gz  package/system/rpcd/Makefile

# BUG: build without static-bin
...
PV="20260122"
PV="2025.12.03"
HASH="ffb9961c1f8bc50830fdd4e144570f11062c2601"  # 2025.12.03
...
pkginst \
  "app-admin/uci" \
  "dev-build/cmake3" \
  "dev-lang/ucode  # (plugin) optional" \
  "dev-libs/json-c" \
  "dev-libs/libubox" \
  "dev-util/pkgconf" \
  "sys-apps/ubus" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  mkdir -m 0755 -- usr/ usr/share/ usr/share/rpcd/ usr/share/rpcd/acl.d/

  cp -v "${PDIR%/}"/files/rpcd.init "${ED}"/etc/init.d/rpcd
  cp -v "${PDIR%/}"/files/rpcd.config "${ED}"/etc/config/rpcd
  cp -v "${PDIR%/}"/files/50-migrate-rpcd-ubus-sock.sh -t "${ED}"/etc/uci-defaults/
  cp -v "${BUILD_DIR}"/unauthenticated.json -t "${ED}"/usr/share/rpcd/acl.d/

  test -d "lib" && mv -v -n lib "$(get_libdir)"
