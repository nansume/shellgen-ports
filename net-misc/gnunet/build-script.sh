...
PN=${PN%%_*}
XPN=${XPN:-$PN}
PV="0.21.2"  # required curl-7.85.0 or never version
PV="0.19.4"
SRC_URI="http://www.nic.funet.fi/pub/gnu/ftp.gnu.org/pub/gnu/gnunet/gnunet-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
pkginst \
  ...
  "net-dns/libidn" \
  "#net-dns/unbound  # deps gnutls for dane" \
  "net-libs/libmicrohttpd  # http?" \
  "net-libs/gnutls" \
  "net-misc/curl" \
  "sys-apps/file" \
  "sys-apps/musl-utils" \
  "sys-devel/binutils" \
  "sys-devel/gcc" \
  "#sys-devel/gettext  # required for nls (optional)" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  rm -- "/$(get_libdir)/libatomic.la"  # fix it: libtool: warning: `/lib*/libatomic.la` seems to be moved
