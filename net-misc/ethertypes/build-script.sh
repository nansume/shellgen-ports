...
# Date: 2025-04-13 20:00 UTC - last change
# Build with useflag: -static -static-libs -shared -lfs -nopie -patch -doc -xstub -diet -musl -stest -strip +noarch

# http://data.gpo.zugaina.org/gentoo/net-misc/ethertypes/ethertypes-0.ebuild
...
XPN=${XPN:-$PN}
PV="2018.03.07"
SRC_URI="https://dev.gentoo.org/~floppym/dist/${PN}-0.gz -> ${PN}-${PV}.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  ${ZCOMP} -dc "${PF}" > "${BUILD_DIR}/${PN}" || exit &&
  printf %s\\n "${ZCOMP} -d ${PF}"

  cd "${BUILD_DIR}/" || die "builddir: not found... error"

  mkdir -pm 0755 -- "${ED}"/etc/
  mv -n ${PN} -t "${ED}"/etc/
