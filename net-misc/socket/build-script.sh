...
# Date: 2025-06-21 18:00 UTC - last change
# Build with useflag: +static -static-libs -shared -lfs +nopie -patch -doc -xstub +diet/+musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/net-misc/socket/socket-1.5.ebuild

# BUG: in dietlibc SO_REUSEADDR no support, here definitely needed disable it.
# FIX: during runtime a program runing is needed make so: [ socket -R -s <port> ] instead [ socket -s <port> ]
# FIX: It needed that disable SO_REUSEADDR there with -R
...
SRC_URI="
  ...
  #http://w21.org/jnickelsen/${PN}/${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/net-misc/socket/files/${PN}-${PV}-makefile.patch
"
USE_BUILD_ROOT="0"
...
pkginst \
  ...
  "sys-devel/gcc14" \
  "sys-devel/make" \
  "sys-libs/musl  # same is here needed for dietlibc" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}"/${PN}-${PV}-makefile.patch
