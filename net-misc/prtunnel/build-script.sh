...
XPN=${XPN:-$PN}
PV="0.2.7"
SRC_URI="https://github.com/joshb/${PN}/releases/download/${PV}/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+ipv6 +static -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's|CFLAGS=|CFLAGS+=|;s|$(CC)|$(CC) $(CFLAGS) $(LDFLAGS)|' -i Makefile || die
  use 'ipv6' || sed "s|CFLAGS+= -DIPV6||;s|direct6\.o:.*||;s|direct6\.o ||" -i Makefile || die
