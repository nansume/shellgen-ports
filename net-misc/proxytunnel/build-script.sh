...
# Date: 2024-08-25 09:00 UTC - last change
# Build with useflag: +ssl +static -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+ssl +static -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e 's/libssl/libssl libcrypto/' -i Makefile || die "Sed failed!"
  use 'ssl' || sed -e '/^OPTFLAGS += -DUSE_SSL$/d' -i Makefile || : die "Sed failed!"
