...
# Date: 2025-05-24 17:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/guru/dev-cpp/simdutf/simdutf-6.2.0.ebuild
...
XPN=${XPN:-$PN}
PV="6.2.0"
SRC_URI="https://github.com/simdutf/simdutf/archive/v${PV}.tar.gz -> ${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -D SIMDUTF_TESTS=$(usex 'test' ON OFF) \
