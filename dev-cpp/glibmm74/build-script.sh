...
# http://data.gpo.zugaina.org/gentoo/dev-cpp/glibmm/glibmm-2.78.1.ebuild
...
PN=${PN%[0-9][0-9]}
PV="2.74.1"
SRC_URI="https://download.gnome.org/sources/glibmm/${PV%.*}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-gtk-doc -debug -test +static-libs +shared -nopie -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-libs/glib74" \
  "dev-libs/libffi  # deps meson,glib" \
  "dev-libs/libsigcxx3" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  sed -e '/giomm_tls_client/d' -i tests/meson.build || die
  ...
  meson setup \
    ...
    -D build-documentation=$(usex 'gtk-doc' true false) \
    -D debug-refcounting=$(usex 'debug' true false) \
  ...
  grep '${prefix}' < $(get_libdir)/pkgconfig/${PN}-${SVER}.pc
  sed -e 's|${prefix}||;s|${exec_prefix}||' -i $(get_libdir)/pkgconfig/*mm-${SVER}.pc || : die

  LD_LIBRARY_PATH="${LD_LIBRARY_PATH:+${LD_LIBRARY_PATH}:}${ED}/$(get_libdir)"
  ldd "$(get_libdir)"/lib${PN}-${SVER}.so.${SYMVER} || : die "library deps work... error"
