...
# Date: 2025-05-31 02:00 UTC - last change
# Build with useflag: -static -static-libs -shared -lfs -nopie +patch -doc -xstub -diet -musl -stest -strip +noarch

# http://data.gpo.zugaina.org/gentoo/dev-cpp/range-v3/range-v3-0.12.0.ebuild
...
PV="0.12.0"
SRC_URI="
  https://github.com/ericniebler/${PN}/archive/${PV}.tar.gz -> ${PN}-${PV}.tar.gz
  http://data.gpo.zugaina.org/gentoo/dev-cpp/${PN}/files/0.12.0-fix-any-1.patch -> ${PN}-0.12.0-fix-any-1.patch
  http://data.gpo.zugaina.org/gentoo/dev-cpp/${PN}/files/0.12.0-fix-any-2.patch -> ${PN}-0.12.0-fix-any-2.patch
"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  patch -p1 -E < "${FILESDIR}/${PN}-0.12.0-fix-any-1.patch"
  patch -p1 -E < "${FILESDIR}/${PN}-0.12.0-fix-any-2.patch"

  # header-only libraries go to arch-independent dirs
  sed -e 's@CMAKE_INSTALL_LIBDIR@CMAKE_INSTALL_DATADIR@g' -i CMakeLists.txt || die
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -D RANGES_NATIVE=OFF \
    -D RANGES_DEBUG_INFO=OFF \
    -D RANGES_NATIVE=OFF \
    -D RANGES_ENABLE_WERROR=OFF \
    -D RANGES_VERBOSE_BUILD=ON \
    -D RANGE_V3_EXAMPLES=OFF \
    -D RANGE_V3_PERF=OFF \
    -D RANGE_V3_DOCS=OFF \
    -D RANGE_V3_HEADER_CHECKS=$(usex 'test' ON OFF) \
    -D RANGE_V3_TESTS=$(usex 'test' ON OFF) \
    -Wno-dev \
