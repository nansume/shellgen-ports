...
# http://data.gpo.zugaina.org/gentoo/dev-cpp/cppgir/cppgir-2.0_p20250508.ebuild
...
XPN=${XPN:-$PN}
PV="2.0p20250508"
TAG="2f8680bb8278b0940c7d2347213fed5bcc098923"
SRC_URI="https://gitlab.com/mnauw/cppgir/-/archive/${TAG}/cppgir-${TAG}.tar.bz2 -> ${PN}-${PV}.tar.bz2"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-doc -test -static +shared (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  append-cppflags -UDEFAULT_GIRPATH
  append-cppflags -DDEFAULT_GIRPATH="${EPREFIX%/}/usr/share:${EPREFIX%/}/usr/local/share"
  ...
  cmake -B build -G "Unix Makefiles" \
    ...
    -D BUILD_DOC=$(usex 'doc' ON OFF) \
    -D BUILD_TESTING=$(usex 'test' ON OFF) \
    -D BUILD_EXAMPLES=no \
    -D INTERNAL_EXPECTED=no \
  ...
  grep 'includedir=${prefix}/include' < $(get_libdir)/pkgconfig/${PN}.pc
  sed -e 's|^includedir=${prefix}/include|includedir=/usr/include|' -i $(get_libdir)/pkgconfig/${PN}.pc
