...
# Date: 2024-11-02 21:00 UTC - last change
...
XPN=${XPN:-$PN}
PV="1.9.16"
SRC_URI="https://github.com/michaelrsweet/htmldoc/releases/download/v${PV}/htmldoc-${PV}-source.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-fltk -ssl +static -doc (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "#x11-base/xorg-proto  # deps: fltk" \
  "#x11-libs/fltk  # fltk (fltk1)" \
  "#x11-libs/libice  # deps: fltk" \
  "#x11-libs/libsm  # deps: fltk" \
  "#x11-libs/libx11  # deps: fltk" \
  "#x11-libs/libxau  # deps: fltk" \
  "#x11-libs/libxcb  # deps: fltk" \
  "#x11-libs/libxcursor  # deps: fltk" \
  "#x11-libs/libxdmcp  # deps: fltk" \
  "#x11-libs/libxext  # deps: fltk" \
  "#x11-libs/libxfixes  # deps: fltk" \
  "#x11-libs/libxft  # deps: fltk" \
  "#x11-libs/libxrender  # deps: fltk" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  for file in configure doc/Makefile doc/htmldoc.man; do
    sed -e "s:/doc/htmldoc:/doc/${PF}/html:g" -i "${file}" ||
  ...
  make DSTROOT="${ED}" STRIPPROG=$(usex 'strip' strip true) install || die "make install... error"

  cd "${ED}/" || die "install dir: not found... error"

  use 'doc' || rm -vfr -- "usr/share/doc/" "usr/share/man/"
