...
# Date: 2024-09-20 13:00 UTC - last change
...
SRC_URI="https://waterlan.home.xs4all.nl/dos2unix/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="-debug -nls -test +static -doc +diet +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  make -j "$(nproc)" ENABLE_NLS= || die "Failed make build"
  sleep 1

  PROG=$(printf %s "${PROG}" | sed 's| |\t|g')

  strip --verbose --strip-all ${PROG}
  ...
  ln -s unix2dos bin/unix2mac
  ln -s dos2unix bin/mac2unix
