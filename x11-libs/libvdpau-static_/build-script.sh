...
# Date: 2025-05-01 19:00 UTC - last change
# Build with useflag: -static +static-libs -shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/gentoo/x11-libs/libvdpau/libvdpau-1.5.ebuild
...
USE="${USE} +static +static-libs -shared"  # force set build-flags
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  CC="gcc -static --static" CXX="g++ -static --static"

  cd "${BUILD_DIR}/" || die "builddir: not found... error"

  sed -e "/^docdir/s|${PN}|${PN}-${PV}|" -i doc/meson.build || die
  ...
  DESTDIR="${ED}" muon -C "${BUILD_DIR}/build" install -d "${ED}" || die "meson install... error"

  cd "${ED}/" || die "install dir: not found... error"

  grep '${prefix}' < $(get_libdir)/pkgconfig/${PN#lib}.pc
