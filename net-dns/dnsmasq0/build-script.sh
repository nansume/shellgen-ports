...
XPN=${XPN:-$PN}
PN=${PN%[0-9]}
PV="2.80"  # TODO: bump to v2.85
PV="2.81"  # TODO: regex patch needed preparation!
SRC_URI="
  https://thekelleys.org.uk/dnsmasq/${PN}-${PV}.tar.xz
  #http://localhost/${PN}-${PV}-p001-regex-server.diff
  #http://localhost/${PN}-${PV}p1-regex-server.diff  # v2.80
"
USE_BUILD_ROOT="0"
...
pkginst \
  "#dev-libs/pcre  # for support regex" \
  "dev-util/pkgconf" \
  "sys-devel/binutils" \
  "sys-devel/gcc14  # or gcc14" \
  "sys-devel/make" \
  "#sys-devel/patch" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  use 'regex' && patch -p1 -E < "${FILESDIR}"/${PN}-${PV}p1-regex-server.diff
  ...
  COPTS="${COPTS} -DNO_DNSSEC -DNO_AUTH -DNO_SCRIPT -DNO_GMP -DNO_LOOP -DNO_DHCP6"
