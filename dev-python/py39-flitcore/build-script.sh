...
# https://crux.nu/ports/crux-3.8/opt/python3-flit-core/Pkgfile
...
PV="3.12.0"
PYVER="3.12"
SRC_URI="https://files.pythonhosted.org/packages/source/f/flit_core/${PN}-${PV}.tar.gz"
USE_BUILD_ROOT="0"
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  # install
  . runverb \
  python3 "bootstrap_install.py" --install-root ${EPREFIX:?} --installdir ${PYTHON_XLIBS} dist/*.whl ||
  python3 -m compileall -d ${PN}-${PV}.src/utils/lit ${ED}
  python3 -O -m compileall -d ${PN}-${PV}.src/utils/lit ${ED}
