...
# Date: 2025-04-14 08:00 UTC - last change
# Build with useflag: -static -static-libs +shared -lfs +nopie -patch -doc -xstub -diet +musl +stest +strip +x32

# http://data.gpo.zugaina.org/guru/net-vpn/fastd/fastd-23.ebuild
...
XPN=${XPN:-$PN}
PV="23"
SRC_URI="https://github.com/neocturne/fastd/releases/download/v${PV}/${PN}-${PV}.tar.xz"
USE_BUILD_ROOT="0"
...
BUILD_USER="tools"
SRC_DIR="build"
IUSE="+caps -doc +offload-l2tp -systemd -test -static +shared +nopie (+musl) +stest +strip"
EABI=$(tc-abi-build)
ABI=${EABI}
XABI=${EABI}
SPREFIX="/"
...
pkginst \
  ...
  "dev-util/pkgconf" \
  "net-libs/libmnl  # offload-l2tp" \
  "net-libs/libuecc  # >=libuecc-6" \
  "sys-devel/binutils" \
  "sys-devel/bison" \
  "sys-devel/gcc9" \
  ...
  "sys-libs/libcap  # caps" \
  "sys-libs/musl0" \
  "sys-libs/zlib  # deps meson,openssl" \
...
if { test "X${USER}" = 'Xroot' && test "${BUILD_CHROOT:=0}" -ne '0' ;} ;then
  ...
  meson setup \
    ...
    -Dbuildtype="release" \
    -Dcapabilities=$(usex 'caps' enabled disabled) \
    -Doffload_l2tp=$(usex 'offload-l2tp' enabled disabled) \
